...
def swipe_action(request):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": "Invalid profile id"}, status=400
        )
    ...
    if action == "like":
        like, created = Like.objects.get_or_create(
            liker=current_user, liked=liked_profile
        )

        # Check if it's a match
        if Like.objects.filter(liker=liked_profile, liked=current_user).exists():
            Match.objects.get_or_create(
                entrepreneur1=current_user, entrepreneur2=liked_profile
            )
            match_found = True
    elif action == "dislike":
    ...
    excluded_profiles = Like.objects.filter(liker=current_user).values_list(
        "liked", flat=True
    )
    ...
    if match_found:
        ...
        response_data["match_profile"] = {
            ...
            or liked_profile.user.username,
...
def matches(request):
    ...
    try:
        ...
        matches = Match.objects.filter(
            Q(entrepreneur1=user_profile) | Q(entrepreneur2=user_profile)
        )
        ...
        return render(
            request,
            "entreprinder/error.html",
            {
                "error_message": "Your entrepreneur profile doesn't exist. Please create one."
            },
        )
    except Exception:
        return render(
            ...
            "entreprinder/error.html",
            {
                "error_message": "An error occurred while loading your matches. Please try again later."
            },
        )
...
def swipe(request):
    ...
    try:
        ...
        if not primary_email.verified:
            messages.warning(
                request,
                "Please verify your email address before you can start swiping.",
            )
            return redirect("entreprinder:profile")
    except EmailAddress.DoesNotExist:
        messages.error(
            request,
            "Could not find your primary email address. Please contact support.",
        )
        return redirect("entreprinder:profile")
    ...
    try:
        ...
        messages.error(
            request, "Please complete your entrepreneur profile before swiping."
        )
        return redirect("entreprinder:profile")

    # Get profiles that haven't been interacted with
    interacted_profiles = Like.objects.filter(liker=current_user).values_list(
        "liked", flat=True
    )
    disliked_profiles = Dislike.objects.filter(disliker=current_user).values_list(
        "disliked", flat=True
    )
    profile_to_display = EntrepreneurProfile.objects.exclude(
        Q(user=request.user)
        | Q(id__in=interacted_profiles)
        | Q(id__in=disliked_profiles)
    ).first()
