...
class ViewsTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.site, created = Site.objects.update_or_create(
            id=1, defaults={"domain": "entreprinder.lu", "name": "Entreprinder"}
        )
...
class MatchingTestCase(TestCase):
    def setUp(self):
        ...
        self.industry = Industry.objects.create(name="Tech")
        self.profile1 = EntrepreneurProfile.objects.create(
            user=self.user1, industry=self.industry
        )
        self.profile2 = EntrepreneurProfile.objects.create(
            user=self.user2, industry=self.industry
        )
    ...
    def test_like_and_match(self):
        ...
        print(
            f"Likes from profile1 to profile2: {Like.objects.filter(liker=self.profile1, liked=self.profile2).exists()}"
        )
        print(
            f"Likes from profile2 to profile1: {Like.objects.filter(liker=self.profile2, liked=self.profile1).exists()}"
        )
    ...
    def test_match_creation_logic(self):
        ...
        Like.objects.create(liker=self.profile1, liked=self.profile2)
        self.assertEqual(
            Match.objects.count(), 0, "Match shouldn't be created after only one like"
        )

        Like.objects.create(liker=self.profile2, liked=self.profile1)
        self.assertEqual(
            Match.objects.count(), 1, "Match should be created after mutual likes"
        )
...
class SwipeActionValidationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.target_user = User.objects.create_user(
            username="targetuser", password="12345"
        )
        self.industry = Industry.objects.create(name="Tech")
        self.user_profile = EntrepreneurProfile.objects.create(
            user=self.user, industry=self.industry
        )
        self.target_profile = EntrepreneurProfile.objects.create(
            user=self.target_user, industry=self.industry
        )
...
class LinkedInOAuthFlowTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Site.objects.update_or_create(
            id=1, defaults={"domain": first_host, "name": first_host}
        )
        ...
        for host in cls.hosts:
            if host == first_host:
                ...
                site, _ = Site.objects.get_or_create(
                    domain=host, defaults={"name": host}
                )
    ...
    def test_login_redirects_use_domain_specific_social_app(self):
        for host in self.hosts:
            with self.subTest(host=host):
                ...
                self.assertEqual(
                    query["client_id"][0], self.social_apps[host].client_id
                )
    ...
    def test_callback_exchanges_code_and_logs_user_in(self):
        ...
        def mock_request(method, url, **kwargs):
            if "accessToken" in url:
                return self._json_response(
                    LinkedInOAuth2Adapter.access_token_url, token_payload
                )
        ...
        self.assertTrue(
            SocialAccount.objects.filter(
                user=user, provider=self.provider_slug
            ).exists()
        )
