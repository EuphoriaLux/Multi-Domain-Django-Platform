    ...
    try:
        ...
        if photo_url and hasattr(sociallogin.user, "id") and sociallogin.user.id:
            ...
            try:
                ...
                profile.save()
                logger.info(
                    f"Updated LinkedIn photo URL for existing user {sociallogin.user.email}"
                )
...
def update_linkedin_photo_on_account_update(sender, request, sociallogin, **kwargs):
    ...
    logger.info(
        "social_account_updated signal received for LinkedIn on entreprinder.lu"
    )
    ...
    try:
        ...
        if photo_url:
            ...
            profile.save()
            logger.info(
                f"Updated LinkedIn photo URL for user {user.email} (created: {created})"
            )
    except Exception as e:
        logger.error(
            f"Error in social_account_updated handler: {str(e)}", exc_info=True
        )
...
def update_linkedin_photo_on_social_account_save(sender, instance, created, **kwargs):
    ...
    if not getattr(_thread_local, "is_entreprinder_login", False):
        logger.info(
            f"Skipping EntrepreneurProfile creation for {instance.user.email} - not an entreprinder.lu login"
        )
        return

    try:
        logger.info(
            f"SocialAccount post_save signal for LinkedIn on entreprinder.lu (user: {instance.user.email})"
        )
        ...
        if photo_url:
            profile, profile_created = EntrepreneurProfile.objects.get_or_create(
                user=instance.user
            )
            profile.linkedin_photo_url = photo_url
            profile.save()
