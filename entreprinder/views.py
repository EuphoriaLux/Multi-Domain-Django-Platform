...
def entrepreneur_list(request):
    ...
    try:
        current_user_profile, created = EntrepreneurProfile.objects.get_or_create(
            user=request.user
        )
        if created:
            return redirect("entreprinder:profile")

        liked_profiles = Like.objects.filter(liker=current_user_profile).values_list(
            "liked_id", flat=True
        )
        profiles = EntrepreneurProfile.objects.exclude(user=request.user).exclude(
            id__in=liked_profiles
        )
        ...
        return render(
            ...
            {
                "error_message": f"An error occurred while loading the entrepreneur list. Please try again later. Error details: {e}"
            },
        )
...
def login_complete(request):
    ...
    if request.user.is_authenticated:
        logger.info(
            f"Authenticated user: {request.user.username} (ID: {request.user.id})"
        )
        ...
        if existing_profile:
            logger.info(
                f"Existing profile found with linkedin_photo_url: {existing_profile.linkedin_photo_url}"
            )
        else:
        ...
        try:
            ...
            logger.info(
                f"All social accounts for user: {[acc.provider for acc in all_accounts]}"
            )

            # Focus only on OpenID Connect provider
            socialaccount = SocialAccount.objects.filter(
                user=request.user, provider="openid_connect_linkedin"
            ).first()

            if socialaccount:
                logger.info(
                    f"Found social account: {socialaccount.provider} (ID: {socialaccount.id})"
                )
                ...
                if linkedin_photo_url:
                    ...
                    profile.save()
                    logger.info(
                        f"Updated profile linkedin_photo_url to: {profile.linkedin_photo_url}"
                    )
                ...
                logger.warning(
                    f"No OpenID Connect LinkedIn account found for user {request.user.username}"
                )
