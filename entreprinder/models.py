...
class EntrepreneurProfile(models.Model):
    ...
    tagline = models.CharField(
        max_length=150, blank=True, help_text="A brief, catchy description of yourself"
    )
...
class Match(models.Model):
    ...
    entrepreneur1 = models.ForeignKey(
        EntrepreneurProfile, on_delete=models.CASCADE, related_name="matches_as_first"
    )
    entrepreneur2 = models.ForeignKey(
        EntrepreneurProfile, on_delete=models.CASCADE, related_name="matches_as_second"
    )
...
class Like(models.Model):
    ...
    liker = models.ForeignKey(
        EntrepreneurProfile, on_delete=models.CASCADE, related_name="likes_given"
    )
    liked = models.ForeignKey(
        EntrepreneurProfile, on_delete=models.CASCADE, related_name="likes_received"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.__class__.objects.filter(
            liker=self.liker, liked=self.liked
        ).exists():
...
class Dislike(models.Model):
    ...
    disliker = models.ForeignKey(
        EntrepreneurProfile, on_delete=models.CASCADE, related_name="dislikes_given"
    )
    disliked = models.ForeignKey(
        EntrepreneurProfile, on_delete=models.CASCADE, related_name="dislikes_received"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.__class__.objects.filter(
            disliker=self.disliker, disliked=self.disliked
        ).exists():
            super().save(*args, **kwargs)
    ...
    if mutual_like:
        try:
            Match.objects.get_or_create(
                entrepreneur1=like.liker, entrepreneur2=like.liked
            )
