...
export class PixelWarColorManager {
    ...
    static updateColorButtonSelection(selectedColor) {
        ...
        document
            .querySelectorAll(".color-btn-enhanced, .recent-color-btn")
            .forEach((btn) => {
    ...
    static updateRecentColorsDisplay() {
        ...
        if (this.recentColors.length === 0) {
            recentColorsList.innerHTML =
                '<p class="no-recent-colors">No recent colors</p>';
            return;
        }
    ...
    static setupSwipeGesture() {
        ...
        document.addEventListener(
            ...
            (e) => {
                ...
                if (
                    touchStartY > window.innerHeight * 0.66 &&
                    deltaY < -50 &&
                    deltaTime < 300
                ) {
                    this.showEnhancedPalette();
