...
class InputHandler extends EventTarget {
    ...
    handleMouseDown(e) {
        const isPanMode =
            this.inputState.spacePressed || e.button === 2 || e.button === 1;
    ...
    handleWheel(e) {
        ...
        if (e.ctrlKey || e.metaKey) {
            ...
            const now = performance.now();
            if (!this.lastWheelTime || now - this.lastWheelTime > 16) {
                // ~60fps throttling
    ...
    handleTouchStart(e) {
        ...
        if (e.touches.length === 1) {
            ...
            this.inputState.isDragging = false;
            this.touchState.touches.clear();
            this.touchState.lastTouchDistance = this.getTouchDistance(
                e.touches[0],
                e.touches[1],
            );
        }
    ...
    handleTouchMove(e) {
        ...
        const now = performance.now();
        if (now - this.lastEventTime < 5) {
            // Balanced 5ms throttling
            return;
        }
        this.lastEventTime = now;
                ...
                if (
                    ...
                    if (Math.random() < 0.01) {
                        ...
                        console.log("📱 Touch Drag:", {
                            ...
                            enhanced:
                                (deltaX * 1.2).toFixed(2) +
                                "," +
                                (deltaY * 1.2).toFixed(2),
        ...
        if (e.changedTouches.length === 1) {
            ...
            if (touchData) {
                ...
                if (duration < 500 && distance < 20 && !this.inputState.isDragging) {
                    ...
                    console.log("❓ Touch gesture not recognized:", {
                        duration,
                        distance,
                        isDragging: this.inputState.isDragging,
                    });
    ...
    screenToCanvas(screenX, screenY) {
        ...
        const finalX = Math.max(
            0,
            Math.min(this.config.width - 1, canvasX - Math.round(this.offsetX)),
        );
        const finalY = Math.max(
            0,
            Math.min(this.config.height - 1, canvasY - Math.round(this.offsetY)),
        );
    ...
    handleLongPress(clientX, clientY) {
        ...
        if (!coords) {
            console.log("❌ Failed to convert long press coordinates:", {
                clientX,
                clientY,
            });
            return;
        ...
        if (
            coords.x >= 0 &&
            coords.x < this.config.width &&
            coords.y >= 0 &&
            coords.y < this.config.height
        ) {
    ...
    handlePixelPreview(clientX, clientY) {
        ...
        if (!coords) {
            console.log("❌ Failed to convert pixel preview coordinates:", {
                clientX,
                clientY,
            });
            return;
    ...
    showPixelConfirmation(x, y, isLongPress) {
        console.log("📱 Creating pixel confirmation modal for:", {
            x,
            y,
