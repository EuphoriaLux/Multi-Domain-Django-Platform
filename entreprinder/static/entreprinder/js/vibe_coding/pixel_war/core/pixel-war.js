...
export class PixelWar {
    constructor(canvasOrId, config) {
        ...
        if (typeof canvasOrId === "string") {
            ...
            throw new Error(
                "First parameter must be either a canvas element or canvas ID string",
            );
        }
        ...
        this.rateLimiter = new RateLimiter(
            config.isAuthenticated
                ? config.registeredPixelsPerMinute
                : config.anonymousPixelsPerMinute,
            config.isAuthenticated
                ? config.registeredCooldown
                : config.anonymousCooldown,
        );
    ...
    setupEventHandlers() {
        ...
        this.inputHandler.addEventListener("drag", (e) => {
            ...
            this.velocityX =
                (e.detail.velocityX || e.detail.deltaX) / (pixelSize * this.zoom);
            this.velocityY =
                (e.detail.velocityY || e.detail.deltaY) / (pixelSize * this.zoom);
        ...
        this.inputHandler.addEventListener("touchdrag", (e) => {
            ...
            if (!shiftKey && !ctrlKey) {
                this.velocityX =
                    (e.detail.velocityX || deltaX) / (pixelSize * this.zoom * 5);
                this.velocityY =
                    (e.detail.velocityY || deltaY) / (pixelSize * this.zoom * 5);
            } else if (shiftKey) {
                // For shift mode, only horizontal velocity
                this.velocityX =
                    (Math.sign(deltaY) * -30) / (pixelSize * this.zoom * 5);
                this.velocityY = 0;
            }
        ...
        this.inputHandler.addEventListener("zoom", (e) => {
            if (e.detail.zoom !== undefined) {
                ...
                if (this.inputHandler.updateViewState) {
                    this.inputHandler.updateViewState(
                        this.zoom,
                        this.offsetX,
                        this.offsetY,
                    );
    ...
    screenToCanvas(screenX, screenY) {
        ...
        if (!rect.width || !rect.height || isNaN(screenX) || isNaN(screenY)) {
            console.warn("❌ Invalid screenToCanvas inputs:", {
                screenX,
                screenY,
                rect,
            });
    ...
    getEffectiveViewport(forceMobile = false) {
        ...
        if (isMobile) {
            ...
            const mobileTopControls =
                document.querySelector(".mobile-controls-bar") ||
                document.getElementById("mobileTopControls");
            const mobileCanvasControls = document.querySelector(
            ...
            if (
                mobileTopControls &&
                getComputedStyle(mobileTopControls).display !== "none"
            ) {
            ...
            if (
                ...
                getComputedStyle(mobileCanvasControls).display !== "none"
            ) {
                bottomControlsHeight += mobileCanvasControls.offsetHeight;
                console.log(
                    "📱 Found mobile canvas controls:",
                    mobileCanvasControls.offsetHeight + "px",
                );
            }

            if (
                mobileActionBar &&
                getComputedStyle(mobileActionBar).display !== "none"
            ) {
                bottomControlsHeight += mobileActionBar.offsetHeight;
                console.log(
                    "📱 Found mobile action bar:",
                    mobileActionBar.offsetHeight + "px",
                );
            }
            ...
            effectiveHeight -=
                topControlsHeight + bottomControlsHeight + mobileChromeMargin;
    ...
    constrainOffsets(forceMobile = false) {
        const {
            ...
            isMobile,
        } = this.getEffectiveViewport(forceMobile);
        const pixelSize = PixelWarConfig.canvas.defaultPixelSize;

        const viewportWidth = effectiveWidth / (pixelSize * this.zoom);
        const viewportHeight = effectiveHeight / (pixelSize * this.zoom);

        this.targetOffsetX = this.constrainOffset(
            this.targetOffsetX,
            this.config.width,
            viewportWidth,
            isMobile,
        );
        this.targetOffsetY = this.constrainOffset(
            this.targetOffsetY,
            this.config.height,
            viewportHeight,
            isMobile,
        ...
        if (Math.random() < 0.2) {
            // Log 20% of the time to debug constraint issues
    ...
    calculateMinZoom() {
        ...
        const safetyMargin = isMobile ? 0.9 : 0.95; // 10% margin for mobile vs 5% for desktop
    ...
    initializeZoom() {
        ...
        console.log("🎯 Initial Zoom Setup:", {
            ...
            reason:
                this.zoom === dynamicMinZoom
                    ? "Using calculated min zoom to fit map"
                    : "Using default zoom",
        });
        ...
        if (
            viewportWidthInGrid >= this.config.width &&
            viewportHeightInGrid >= this.config.height
        ) {
        ...
        console.log(
            "🎯 Initial Position: Centered at",
            `(${this.offsetX.toFixed(2)}, ${this.offsetY.toFixed(2)})`,
        );
    ...
    navigateToCorner(corner) {
        ...
        this.notifications.show(
            `Navigating to ${corner === "center" ? "center" : corner + " corner"}`,
            "info",
            2000,
        );
    }
    ...
    animate() {
        ...
        const stillMoving =
            Math.abs(this.targetOffsetX - this.offsetX) > 0.01 ||
    ...
    applyMomentum() {
        const animate = () => {
            ...
            const threshold = Math.max(
                0.01,
                PixelWarConfig.animation.momentumThreshold,
            );
            if (
                Math.abs(this.velocityX) > threshold ||
                Math.abs(this.velocityY) > threshold
            ) {
    ...
    render() {
        ...
        if (Math.random() < 0.001) {
            // Log 0.1% of renders to avoid spam
    ...
    async placePixel(x, y) {
        if (!this.rateLimiter.canPlacePixel()) {
            ...
            if (cooldownRemaining > 0) {
                this.notifications.show(
                    `Wait ${Math.ceil(cooldownRemaining)}s before placing another pixel`,
                    "warning",
                );
            } else {
                const timeLeft = Math.ceil(this.rateLimiter.getTimeUntilReset());
                this.notifications.show(
                    `Rate limit reached. Reset in ${timeLeft}s`,
                    "warning",
                );
            }
            return;
        ...
        try {
            const response = await this.api.placePixel(
                x,
                y,
                this.selectedColor,
                this.config.id,
            ...
            if (response.success) {
                this.renderer.updatePixel(
                    x,
                    y,
                    response.pixel.color,
                    response.pixel.placed_by,
                );
                ...
                this.notifications.show(
                    `Pixel placed! (${remaining} remaining)`,
                    "success",
                );
                this.updateUI();
            }
    ...
    updateUI() {
        ...
        if (timer) {
            if (cooldownRemaining > 0) {
                ...
                const progressBar =
                    timer.parentElement?.querySelector(".cooldown-progress");
                if (progressBar) {
                    const progress =
                        ((this.rateLimiter.cooldownSeconds - cooldownRemaining) /
                            this.rateLimiter.cooldownSeconds) *
                        100;
        ...
        if (remaining) {
            remaining.innerHTML = `
                <div class="pixels-info">
                    ...
                    ${
                        timeUntilReset > 0 &&
                        pixelsRemaining < this.rateLimiter.maxPixelsPerMinute
    ...
    setupTouchModeToggle() {
        ...
        window.debugCentering = () => {
            ...
            const expectedOffsetX = canFitWidth
                ? -(viewportWidth - this.config.width) / 2
                : this.targetOffsetX;
            const expectedOffsetY = canFitHeight
                ? -(viewportHeight - this.config.height) / 2
                : this.targetOffsetY;
            ...
            if (canFitWidth) {
                ...
                console.log(
                    `Width centering: viewport=${viewportWidth.toFixed(2)}, grid=${this.config.width}, offset=${testOffsetX.toFixed(2)}`,
                );
            }
            if (canFitHeight) {
                const testOffsetY = -(viewportHeight - this.config.height) / 2;
                console.log(
                    `Height centering: viewport=${viewportHeight.toFixed(2)}, grid=${this.config.height}, offset=${testOffsetY.toFixed(2)}`,
                );
            }
        };
        ...
        window.fullDebug = () => {
            ...
            if (viewportWidth < this.config.width) {
                ...
                console.log(
                    "   Need zoom <= " +
                        (effectiveWidth / (this.config.width * pixelSize)).toFixed(3),
                );
            }
            if (viewportHeight < this.config.height) {
                console.log(
                    "❌ HEIGHT PROBLEM: Viewport height " +
                ...
                console.log(
                    "   Need zoom <= " +
                        (effectiveHeight / (this.config.height * pixelSize)).toFixed(3),
                );
            }
        };
                ...
                expectedCenterOffsets: {
                    x:
                        viewportWidth >= this.config.width
                            ? (-(viewportWidth - this.config.width) / 2).toFixed(2)
                            : "N/A",
                    y:
                        viewportHeight >= this.config.height
                            ? (-(viewportHeight - this.config.height) / 2).toFixed(2)
                            : "N/A",
                },
            });
        ...
        const canvasSection =
            ...
            document.querySelector(".pixel-war-container") ||
            document.body;
        ...
        if (btn) {
            btn.addEventListener("click", () => {
                this.inputHandler.touchMode =
                    this.inputHandler.touchMode === "tap" ? "precision" : "tap";
