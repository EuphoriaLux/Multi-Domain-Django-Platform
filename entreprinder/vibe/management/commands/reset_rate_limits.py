    ...
    def handle(self, *args, **options):
        ...
        if options["canvas_id"]:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'❌ PixelCanvas with ID {options["canvas_id"]} not found'
                    )
                )
                return
        else:
        ...
        for canvas in canvases:
            ...
            if not cooldowns:
                self.stdout.write(
                    f"   📭 No cooldown records found for {target_description}"
                )
                continue

            self.stdout.write(
                f"   🔍 Found {len(cooldowns)} cooldown record(s) for {target_description}"
            )
            ...
            for cooldown in cooldowns:
                user_id = (
                    cooldown.user.username
                    if cooldown.user
                    else f"session:{cooldown.session_key[:8]}"
                )
            ...
            if reset_count == 0:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"   ✅ Successfully reset {reset_count} record(s)"
                    )
                )
        ...
        self.stdout.write(
            "   python manage.py reset_rate_limits --all-users --expired-only"
        )
        self.stdout.write("")
