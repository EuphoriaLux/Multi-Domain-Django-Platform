...
def pixel_war(request):
    ...
    if request.user.is_authenticated:
        user_stats, created_stats = UserPixelStats.objects.get_or_create(
            user=request.user, canvas=canvas
        )
    ...
    try:
        ...
        try:
            ...
            if request.user.is_authenticated:
                ...
                cooldowns = UserPixelCooldown.objects.filter(
                    user=None, canvas=canvas, session_key=session_key
                ).order_by("-last_placed")
                cooldown = cooldowns.first()
                if cooldowns.count() > 1:
                    cooldowns.exclude(id=cooldown.id).delete()
        ...
        return JsonResponse(
            {
                ...
                "pixel": {
                    ...
                    "color": pixel.color,
                    "placed_by": (
                        pixel.placed_by.username if pixel.placed_by else "Anonymous"
                    ),
                },
                "cooldown_info": {
                    "cooldown_seconds": 0,
                    "pixels_remaining": max_pixels_per_minute
                    - cooldown.pixels_placed_last_minute,
                    "is_registered": request.user.is_authenticated,
                },
            }
        )
        ...
        return JsonResponse(
            {"error": "An error occurred while placing the pixel"}, status=500
        )
