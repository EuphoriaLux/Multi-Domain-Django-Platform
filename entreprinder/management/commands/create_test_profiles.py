...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "total", type=int, help="Indicates the number of profiles to be created"
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        for i in range(total):
            ...
            profile_pic_path = os.path.join(
                settings.MEDIA_ROOT, "profile_pics", f"Test{i % 9 + 1}.jpg"
            )
            ...
            if os.path.exists(profile_pic_path):
                with open(profile_pic_path, "rb") as f:
                    profile.profile_picture.save(
                        f"Test{i % 9 + 1}.jpg", File(f), save=True
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Successfully created {total} test profiles")
        )
