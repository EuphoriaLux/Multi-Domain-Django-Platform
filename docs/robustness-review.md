...
- **M18 — Tester/payment waitlist flags are vestigial admin-only tracking (corrected — not a user-facing dead-end)** (`crush_lu/models/crush_connect.py:45-52`). `selected_as_tester`/`payment_confirmed` are set by staff and passed to the teaser context (`views_static.py:234-235`), but the teaser renders **no** tester/beta/waitlist UI — `test_crush_connect.py:1206-1224` asserts a selected, payment-confirmed tester sees only the normal status block. So there is no user-visible monetized dead-end; the beta framing is retired. Residual (process, not code): the flags grant no access, so tester payments shouldn't be collected out-of-band until access is wired (see R2's tier field).
...
**R2 — One eligibility predicate + one queryset annotation for Crush Connect.** *Root cause of M16, M18, L7. (M7 is a concurrency race — closed by its own row-lock / partial-unique-constraint, not by this redesign.)* "Surfaceable" is currently defined 3–4 times in incompatible shapes: ORM filters (`get_eligible_pool`), per-object predicates (`is_catalogue_eligible`, `is_sender_eligible`, `_user_is_connect_receiver_eligible`, …), and render-time re-filters. New prerequisites get threaded through some and forgotten in others (photo-consent everywhere, gate-questions nowhere in the pool). **Better:** a single `catalogue_ready(user) -> (bool, reason)` + a matching queryset annotation, both generated from one shared `(field, required_value)` constant tuple. Add an explicit `tier ∈ {candidate, premium, tester}` field so tester access can be granted explicitly if the beta needs it.

**R3 — Model the read-the-photo gate as an explicit two-cell directional state.** Today a single `CuriositySpark.status` is overloaded to encode a two-directional read, forcing the two directions (`send_spark` forward vs `respond_to_spark` answer-back) through different branches with different eligibility rules — the M-tier answer-back asymmetry. **Better:** a `GateRead(reader, owner, aligned)` record with `matched` derived from both cells, making the two directions symmetric by construction. *(Correction: stale-answer handling is already correct — `forms_crush_connect.py:477-491` clears a viewer's guesses when the owner drops a question or changes its truth, keeping only same-truth guesses; an earlier draft wrongly claimed a changed gate stays permanently locked, so that is not a justification for this redesign.)*
