...
### C1 — Unauthenticated exposure of Azure billing data *(adjacent app: `power_up/finops`, not `crush_lu` — but same Azure deployment)* — ⚠️ OPEN (fix proposed in #572, not yet merged)
Several FinOps Hub endpoints serve the operator's internal Azure billing data (subscription/service names, resource IDs, per-service spend, and a CSV export) without an effective authentication check: the dashboard views (`power_up/finops/views.py`) carry no auth decorator, and the DRF cost APIs use a **fail-open** permission class (`power_up/finops/permissions.py`) that admits ordinary browser requests. Because all sites share one `User` table, the exposure is not limited to staff.
**Remediation (proposed in #572 — awaiting merge + deploy; NOT present in this tree):** `@staff_member_required` on every dashboard view; `IsAdminOrStaff` + `SessionAuthentication` on every ViewSet/API endpoint; the fail-open permission classes deleted; permission tests rewritten to assert the lockdown. **Until #572 lands and deploys, treat this as an active exposure.**
