...
### C1 — Unauthenticated exposure of Azure billing data *(adjacent app: `power_up/finops`, not `crush_lu` — but same Azure deployment)* [verified]
`power-up.lu/finops/` dashboards (`power_up/finops/views.py:40 dashboard`, plus subscription/service/resource/anomaly/forecast views) carry **no auth decorator**, and the JSON/CSV API uses `HasSessionOrIsAuthenticated` (`power_up/finops/permissions.py:35`) which returns `True` for any request sending `Accept: text/html` (`:60`) — a fail-open anti-pattern. Because all 9 sites share one `User` table, the CSV export (`power_up/finops/api_views.py:408`) is also dumpable by any logged-in crush.lu member. Exposes subscription names, resource IDs, and per-service spend. (`azureproject/views_seo.py:71` even `Disallow: /finops/` in robots.txt — it is known to be public and merely hidden from crawlers.)
**Fix:** apply the app's own `IsAdminOrStaff` to every finops ViewSet/API + `@staff_member_required` on the dashboards; **delete `HasSessionOrIsAuthenticated`**. ~1 hour.
*Outside the stated crush.lu scope, but it is the single most severe issue on the instance and leads for that reason.*
...
`save_profile_step3` (`crush_lu/views_profile.py:523-534`) assigns `profile.photo_1 = request.FILES[...]` directly; the thorough `_validate_photo` (`crush_lu/forms.py:416-517`: 10 MB cap, MIME sniff, `Image.verify()`, 5000px bomb guard) is only wired to the form path and the draft endpoint, not here. With JS off, a 200 MB TIFF or a decompression-bomb PNG is stored and later decoded by thumbnailing/coach review → memory-exhaustion DoS.
**Fix:** route every photo write through the same validator used in `upload_photo_draft`.

### H2 — Referral reward double-award race (no row lock on the guard)
`apply_referral_reward` (`crush_lu/referrals.py:169-218`) reads `attribution.reward_applied` **outside** any lock, then credits points with `F()`. Reachable concurrently from the login signal, coach approval, and check-in. Two threads can both pass the guard → referrer credited twice for one signup (points map to €-discounts, so this is financial).
...
- **M2 — Connect 7-step wizard finalizes on the step pointer, validating only step 7** (`crush_lu/views_crush_connect.py:346-369`). Steps 1–6 are not re-asserted, so a partially-populated membership can enter the match pool.
...
- **M11 — CSP is report-only, and `unsafe-inline` is in `script-src`** [verified] (`azureproject/settings.py:1038,1044`; prod confirmed report-only at `azureproject/production.py:600`). Today there is **no CSP-based XSS mitigation at all** — the bigger issue than `unsafe-inline`. Fix order: migrate inline HTMX/Alpine handlers to nonces → remove `unsafe-inline` → flip `SECURE_CSP_REPORT_ONLY` to `SECURE_CSP`. Enforcing while `unsafe-inline` remains buys nothing.
...
- **M15 — WhatsApp send has no retry/backoff and no dedup** (`hub/views_whatsapp.py`); rate-limiting exists only on OTP send, not the general send path. A transient Meta error marks a message FAILED permanently → users cannot complete verification. Fix: bounded retry with backoff on 5xx; idempotency key to prevent double-send.
...
- **M20 — In-memory channel layer silently breaks broadcasts under multi-worker** (`azureproject/settings.py:255`). If prod ever runs multi-worker with `REDIS_URL` unset/typo'd, half the room's screens freeze with no error. Fix: hard-fail in `production.py` if the channel backend is not `channels_redis`.
