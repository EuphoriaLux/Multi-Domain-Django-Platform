...
(function () {
    ...
    // Declared up here on purpose. `const` has no hoisting you can use: referencing
    // it above its declaration is a ReferenceError that kills this whole IIFE at
    // parse-time-adjacent runtime, and the page renders its static zeros forever.
    ...
    function fmt(n) {
        ...
        const s =
            x < 10 ? x.toFixed(2) : x < 100 ? x.toFixed(1) : String(Math.floor(x));
        return s + UNITS[u];
    }
        ...
        if (data) {
            ...
            if (r.points > 0) {
                floatText(
                    ...
                    "+" + fmt(r.points) + " 💘",
                    action === "like"
                        ? "var(--color-empire-green)"
                        : "var(--color-empire-pink2)",
                );
            } else if (r.points < 0) {
                floatText(
    ...
    /**
     * How each ending plays.
     *
     * `flee` decides whether the fish escapes, and it is the same question the
     * card asked: did you stop it? Catching it pins it in place. Letting it
     * through — by liking it, or by quietly noping and telling nobody — lets it
     * swim off with whatever it took.
     */
    const ENDINGS = {
        correct: {
            stamp: "stampGotcha",
    ...
    function particle(glyph, cls, dx, dy, delay, spin) {
        const el = document.createElement("div");
        el.className = `unmask-particle ${cls}`;
        el.textContent = glyph;
        // CSSOM, not setAttribute("style") — the latter is what CSP blocks.
        el.style.setProperty("--dx", `${dx}px`);
        el.style.setProperty("--dy", `${dy}px`);
        el.style.setProperty("--spin", `${spin || 0}deg`);
        el.style.animationDelay = `${delay}ms`;
        return el;
    }

    function bubble(x, delay) {
        const el = document.createElement("div");
        el.className = "unmask-bubble go";
        el.textContent = "○";
        el.style.left = `${x}%`;
        el.style.animationDelay = `${delay}ms`;
        return el;
    ...
    function playUnmask(outcome) {
        ...
        const { left, right, crack, fish, shades, stamp, wave } = stageNodes;

        // Reset: strip the animation classes and drop any particles from last time.
        stage
            .querySelectorAll(".unmask-particle, .unmask-bubble")
            .forEach((n) => n.remove());
        Object.values(stageNodes).forEach((n) =>
            n.classList.remove("go", "caught", "flee"),
        );
        stage.hidden = false;
        ...
        // Force a reflow so the freshly-removed animations actually restart. Without
        // this, the second scam card in a row shows a static fish.
        void stage.offsetWidth;

        crack.classList.add("go");
        left.classList.add("go");
        right.classList.add("go");
        // Exactly one of caught/flee — never both plus `go`, or which animation wins
        // would depend on CSS source order.
        fish.classList.add(ending.flee ? "flee" : "caught");
        if (ending.shades) shades.classList.add("go");
        // The stamp's 820ms delay is in CSS, so the whole sequence is one timeline.
        ...
        if (ending.burst) {
            // Flags fly out of the fish, one per red flag we're about to explain.
            for (let i = 0; i < 10; i++) {
        ...
        if (outcome === "catfished") {
            // Your crushes fly INTO its mouth. It came for the money, not for you.
            for (let i = 0; i < 7; i++) {
                const angle = Math.PI * (0.75 + (i / 6) * 0.5);
                stage.appendChild(
                    particle(
                        "💘",
                        "steal",
                        Math.cos(angle) * 90,
                        Math.sin(angle) * 90 - 20,
                        700 + i * 55,
                    ),
                );
            }
            revealEl.firstElementChild.classList.remove("empire-shake");
            void revealEl.firstElementChild.offsetWidth;
            revealEl.firstElementChild.classList.add("empire-shake");
        }

        if (ending.flee) {
            for (let i = 0; i < 5; i++) {
                stage.appendChild(bubble(46 + i * 5, 1150 + i * 70));
            }
        }
    }
    ...
    function announce(r) {
        ...
        r.reveal.flags.forEach((f) => {
            ...
            if (r.tier === 2 && f.text) {
                ...
                line.className =
                    "font-semibold " +
                    (caught ? "text-empire-green" : "text-empire-red");
        ...
        // Only now. CSS animations do not run inside a `display: none` subtree,
        // and adding the classes while the modal is still hidden leaves the fish
        // and the stamp permanently at their opacity-0 start frame. rAF gives the
        // browser one frame to lay the modal out before the timeline starts.
        requestAnimationFrame(() => playUnmask(r.outcome));
    }
