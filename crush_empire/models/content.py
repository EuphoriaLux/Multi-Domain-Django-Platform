...
class GameProfile(models.Model):
    """
    One card in the deck. Generated illustrated portraits (or emoji), never
    photographs — no likeness rights, no GDPR exposure, and no dating game
    built out of real faces. Portraits are DiceBear "lorelei" (CC0) SVGs,
    derived deterministically from `avatar_seed`, generated by
    `manage.py generate_empire_avatars` and committed as static files —
    self-hosted, nothing fetched at runtime.
    ...
    The portrait must never correlate with `is_scam` either — a distinctive
    "scam look" would hand the player the answer before they read a word.
    """
    ...
    avatar_seed = models.CharField(
        ...
        default="",
        help_text=_(
            "Deterministic portrait seed (usually the name). Change it to "
            "re-roll the face, then run manage.py generate_empire_avatars "
            "and commit the SVG. Blank = emoji-only card."
        ),
    )
    ...
    def avatar_static_path(self):
        """Static-relative path of the generated portrait, or None (emoji card)."""
        if not self.avatar_seed:
            return None
        return f"crush_empire/avatars/{slugify(self.avatar_seed)}.svg"
