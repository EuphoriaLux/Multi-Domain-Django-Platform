...
class VdlCategory(models.Model):
    ...
    slug = models.SlugField(
        max_length=120, unique=True, help_text="URL-friendly version of the name"
    )
...
class VdlProducer(models.Model):
    ...
    is_featured_on_homepage = models.BooleanField(
        default=False, help_text="Feature this producer on the homepage?"
    )

    # Vineyard characteristics
    vineyard_size = models.CharField(
        max_length=50,
        blank=True,
        null=True,
        help_text="Total vineyard size (e.g., '25 hectares')",
    )
    elevation = models.CharField(
        max_length=50,
        blank=True,
        null=True,
        help_text="Vineyard elevation (e.g., '150m')",
    )
    soil_type = models.CharField(
        max_length=100,
        blank=True,
        null=True,
        help_text="Predominant soil type (e.g., 'Clay-limestone')",
    )
    sun_exposure = models.CharField(
        max_length=50,
        blank=True,
        null=True,
        help_text="Primary sun exposure (e.g., 'South-facing')",
    )

    # Map positioning for interactive vineyard map
    map_x_position = models.IntegerField(
        default=50, help_text="X position on vineyard map (0-100%)"
    )
    map_y_position = models.IntegerField(
        default=50, help_text="Y position on vineyard map (0-100%)"
    )

    # Additional vineyard features
    vineyard_features = models.JSONField(
        default=list,
        blank=True,
        help_text="List of vineyard features (e.g., ['Organic certification', 'Historic estate', 'Award-winning'])",
    )
...
class BaseProduct(models.Model):
    ...
    short_description = models.TextField(
        help_text="Brief overview for product listings."
    )
    full_description = models.TextField(
        blank=True, null=True, help_text="Detailed description for product page."
    )
...
class VdlCoffret(BaseProduct):
    ...
    price = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        help_text="Price for a single, one-time purchase of this coffret.",
    )
    stock_quantity = models.PositiveIntegerField(
        default=0, help_text="Available stock for this coffret"
    )
...
class VdlAdoptionPlan(BaseProduct):
    ...
    price = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        help_text="Price for the entire adoption package",
    )

    # ADD DEFAULTS HERE
    duration_months = models.IntegerField(
        help_text="Duration of the adoption in months", default=12
    )
    coffrets_per_year = models.IntegerField(
        help_text="Number of seasonal coffrets included per year", default=1
    )

    includes_visit = models.BooleanField(
        default=False, help_text="Does this plan include a vineyard visit?"
    )
    visit_details = models.TextField(
        blank=True, null=True, help_text="Details about the vineyard visit"
    )
    includes_medallion = models.BooleanField(
        default=False, help_text="Does this plan include a personalized medallion?"
    )
    includes_club_membership = models.BooleanField(
        default=False, help_text="Does this plan include Club Cuvée Privée membership?"
    )
    avant_premiere_price = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        blank=True,
        null=True,
        help_text="Price for the Avant-Première gift option",
    )
    welcome_kit_description = models.TextField(
        blank=True,
        null=True,
        help_text="Description of what's included in the welcome kit",
    )
...
class VdlProductImage(models.Model):
    ...
    content_type = models.ForeignKey(
        ...
        help_text="The type of product this image is for.",
        limit_choices_to={"model__in": ("vdlcoffret", "vdladoptionplan")},
    )
    object_id = models.PositiveIntegerField(
        help_text="The ID of the specific product instance."
    )
    ...
    alt_text = models.CharField(
        max_length=100, blank=True, help_text="Description for SEO and accessibility"
    )
...
class VdlOrder(models.Model):
    ...
    notes = models.TextField(
        blank=True, null=True, help_text="Customer notes or internal notes"
    )
...
class HomepageContent(models.Model):
    hero_title = models.CharField(
        max_length=200, help_text="Title for the homepage hero section"
    )
...
class VdlAdoptionPlanImage(models.Model):
    ...
    order = models.IntegerField(
        default=0, help_text="Display order for rotation (lower numbers appear first)"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_primary:
            VdlAdoptionPlanImage.objects.filter(
                adoption_plan=self.adoption_plan, is_primary=True
...
class VdlPlot(models.Model):
    ...
    name = models.CharField(
        max_length=200, help_text="Plot name (e.g., 'Hillside Premier')"
    )
    plot_identifier = models.CharField(
        max_length=50, unique=True, help_text="Unique plot identifier (e.g., 'PLT-001')"
    )
    producer = models.ForeignKey(
        VdlProducer,
        on_delete=models.CASCADE,
    ...
    plot_size = models.CharField(
        max_length=50, help_text="Plot size (e.g., '0.25 hectares', '2,500 sq meters')"
    )
    elevation = models.CharField(
        max_length=50,
        blank=True,
    ...
    vine_age = models.IntegerField(
        blank=True, null=True, help_text="Average age of vines in years"
    )
    harvest_year = models.IntegerField(
        blank=True, null=True, help_text="Expected harvest year for adoption plans"
    )

    # Wine characteristics
    wine_profile = models.TextField(
        blank=True, null=True, help_text="Expected wine characteristics from this plot"
    )
    ...
    base_price = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Base adoption price for this plot"
    )
    ...
    is_premium = models.BooleanField(
        default=False, help_text="Is this a premium plot with special characteristics?"
    )
...
class VdlPlotReservation(models.Model):
    ...
    plot = models.ForeignKey(
        VdlPlot, on_delete=models.CASCADE, related_name="reservations"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="plot_reservations"
    )
    adoption_plan = models.ForeignKey(
        VdlAdoptionPlan,
        on_delete=models.CASCADE,
    ...
    is_confirmed = models.BooleanField(
        default=False, help_text="Has this reservation been confirmed as an adoption?"
    )
    confirmation_date = models.DateTimeField(blank=True, null=True)

    # Additional reservation data
    notes = models.TextField(
        blank=True, null=True, help_text="User notes or special requests"
    )
    session_data = models.JSONField(
        default=dict, help_text="Session data from the selection process"
    )
