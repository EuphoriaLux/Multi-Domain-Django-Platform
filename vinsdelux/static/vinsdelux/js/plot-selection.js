...
class PlotSelection {
    ...
    addPlot(plot) {
        ...
        this.showNotification(
            `${plot.name || "Plot"} added to your selection`,
            "success",
        );
    ...
    updateCartSidebar() {
        ...
        const plotsHTML = Array.from(this.selectedPlots.values())
            .map((plot) => this.createPlotCartItem(plot))
            .join("");
    ...
    getCartSummary() {
        ...
        return {
            totalPlots,
            totalValue:
                totalValue > 0 ? `€${totalValue.toLocaleString()}` : "Price on request",
            averagePrice:
                totalValue > 0 && plotsWithPrice.length > 0
    ...
    animateCartRemoval(plotId) {
        const cartItem = document.querySelector(
            `.plot-cart-item[data-plot-id="${plotId}"]`,
        );
        if (!cartItem) return;
    ...
    async proceedToCheckout() {
        if (this.selectedPlots.size === 0) {
            this.showNotification(
                "Please select at least one plot to proceed",
                "warning",
            );
            return;
        }
    ...
    async validateSelection() {
        try {
            ...
            if (!result.valid) {
                this.showNotification(
                    result.message || "Some plots are no longer available",
                    "error",
                );
                return false;
            }
    ...
    loadFromSession() {
        try {
            ...
            if (data) {
                ...
                this.selectedPlots = new Map(
                    Object.entries(parsed.plots).map(([k, v]) => [parseInt(k), v]),
                );
            }
        } catch (error) {
