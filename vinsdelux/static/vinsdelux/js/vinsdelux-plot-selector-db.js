...
class VinsDeluxPlotSelectorDB {
    ...
    async loadAdoptionPlans() {
        try {
            ...
            const langPrefix =
                pathParts[1] && pathParts[1].length === 2 ? `/${pathParts[1]}` : "";
    ...
    renderPlotCards() {
        ...
        if (this.adoptionPlans.length === 0) {
            container.innerHTML =
                '<p class="no-plots-message">No adoption plans available at this time.</p>';
            return;
        }
    ...
    createPlanCard(plan) {
        ...
        if (selectBtn) {
            selectBtn.addEventListener("click", () =>
                this.selectPlan(plan.id || plan.plan_id),
            );
        }
    ...
    handleMapClick(e) {
        ...
        const clickedPlan = this.adoptionPlans.find((plan) => {
            ...
            const distance = Math.sqrt(
                Math.pow(x - plan._mapX, 2) + Math.pow(y - plan._mapY, 2),
            );
            return distance < 25;
        });
    ...
    showPlanDetails(plan) {
        ...
        const features = [];
        if (plan.includes_visit)
            features.push({
                icon: "fa-map-marked-alt",
                text: "Vineyard Visit Included",
            });
        if (plan.includes_medallion)
            features.push({ icon: "fa-medal", text: "Personalized Medallion" });
        if (plan.includes_club_membership)
            features.push({ icon: "fa-users", text: "Club Membership" });
    ...
    setupFilters() {
        ...
        const regions = [
            ...new Set(
                this.adoptionPlans
                    .map((p) => p.region || p.producer_name)
                    .filter(Boolean),
            ),
        ];
    ...
    applyFilters() {
        ...
        const medallionChecked = document.querySelector(
        ...
        const filteredPlans = this.adoptionPlans.filter((plan) => {
            if (region && plan.region !== region && plan.producer_name !== region)
                return false;
    ...
    resetFilters() {
        document
            .querySelectorAll(".plot-filters select")
            .forEach((select) => (select.value = ""));
        document
            .querySelectorAll('.plot-filters input[type="checkbox"]')
    ...
    setupEventListeners() {
        ...
        document
            ...
            ?.addEventListener("click", () => this.zoomMap(1.2));
        document
            .querySelector(".map-zoom-out")
            ?.addEventListener("click", () => this.zoomMap(0.8));
        document
            .querySelector(".map-reset")
            ?.addEventListener("click", () => this.resetMap());
    }
    ...
    confirmSelection() {
        ...
        const plan = this.adoptionPlans.find(
            (p) => (p.id || p.plan_id) === this.selectedPlan,
        );
