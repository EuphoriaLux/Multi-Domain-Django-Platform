...
class VinsDeluxJourneyGame {
    ...
    showPlotDetails(plotId) {
        ...
        this.safeUpdateElement(
            "#plot-size",
            `${Math.floor(Math.random() * 5 + 2)} hectares`,
        );
        this.safeUpdateElement(
            "#plot-elevation",
            `${Math.floor(Math.random() * 300 + 100)}m`,
        );
    ...
    setupProductionTimeline() {
        this.safeAddEventListener(".btn-play-timeline", "click", () =>
            this.playTimeline(),
        );
    ...
    updatePackageContents(items) {
        ...
        contentsList.innerHTML = items
            .map((item) => `<li class="content-item">${item}</li>`)
            .join("");
    }
    ...
    selectLegacy(type) {
        ...
        const selectedCard = document.querySelector(
            `.legacy-card .btn-select-legacy[data-legacy="${type}"]`,
        );
        if (selectedCard) {
    ...
    updateJourneySummary() {
        ...
        this.safeUpdateElement(
            "#selected-plot-name",
            `Plot ${this.journeyData.plot || "Not Selected"}`,
        );
    ...
    completeJourney() {
        ...
        this.showNotification(
            "🎉 Congratulations! Your wine journey is complete!",
            "success",
        );
    ...
    setupImageErrorHandlers() {
        ...
        const observer = new MutationObserver(() => {
            document
                .querySelectorAll("img:not([data-error-handler])")
                .forEach((img) => {
