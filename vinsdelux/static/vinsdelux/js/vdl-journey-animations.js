    ...
    initStepAnimation(stepIndex, ctx, canvas) {
        ...
        switch (stepIndex) {
            case 0: // Plot Selection - Vineyard Animation
                this.animations.set(
                    stepIndex,
                    new VineyardAnimation(ctx, width, height),
                );
                break;
            case 1: // Personalize Your Wine - Customization Animation
                this.animations.set(
                    stepIndex,
                    new CustomizationAnimation(ctx, width, height),
                );
                break;
            case 2: // Follow Production - Monitoring Animation
                this.animations.set(
                    stepIndex,
                    new MonitoringAnimation(ctx, width, height),
                );
                break;
            case 3: // Receive and Taste - Delivery Animation
                this.animations.set(
                    stepIndex,
                    new DeliveryAnimation(ctx, width, height),
                );
                break;
    ...
    update(deltaTime = 16) {
        ...
        if (!this.performanceMode || Math.floor(this.time * 10) % 2 === 0) {
            this.vines.forEach((vine) => {
                const growthTarget =
                    vine.maxHeight *
                    (0.5 + 0.5 * Math.sin(this.time * vine.growth + vine.phase));
...
class CustomizationAnimation {
    ...
    draw() {
        ...
        if (this.wineLevel > 0) {
            ...
            const wineGradient = this.ctx.createLinearGradient(
                0,
                glassBottom - 20,
                0,
                glassTop,
            );
            wineGradient.addColorStop(
                0,
                this.interpolateColor(currentColor, nextColor, this.colorTransition),
            );
            wineGradient.addColorStop(
                1,
        ...
        this.bubbles.forEach((bubble) => {
            if (bubble.y < glassBottom - 20 && bubble.y > glassTop) {
                ...
                bubbleGradient.addColorStop(
                    0,
                    `rgba(212, 175, 55, ${bubble.opacity * 0.8})`,
                ); // wine-accent core
                bubbleGradient.addColorStop(
                    0.7,
        ...
        this.colorPalette.forEach((color, index) => {
            ...
            const swatchGradient = this.ctx.createRadialGradient(
                x,
                paletteY,
                0,
                x,
...
class DeliveryAnimation {
    ...
    update() {
        ...
        const truckProgress = Math.max(
            0,
            Math.min(1, (this.truck.x + 100) / (this.width + 200)),
        );
    ...
    draw() {
        ...
        // Draw elegant road
        const roadGradient = this.ctx.createLinearGradient(
            0,
            this.height * 0.75,
            0,
            this.height,
        ...
        this.trackingDots.forEach((dot, index) => {
            ...
            if (dot.progress > 0) {
                ...
                this.ctx.arc(
                    ...
                    12,
                    -Math.PI / 2,
                    -Math.PI / 2 + dot.progress * Math.PI * 2,
                );
                this.ctx.stroke();
            }
    ...
    initTree() {
        ...
        this.tree.branches.forEach((branch, branchIndex) => {
            ...
            for (let i = 0; i < bottleCount; i++) {
                ...
                const x =
                    branch.startX + Math.cos(branch.angle) * branch.length * progress;
                const y =
                    branch.startY + Math.sin(branch.angle) * branch.length * progress;
