...
class DebugConsole {
    ...
    highlightContainers() {
        const containers = document.querySelectorAll(
            ".journey-steps-container, .step-card-container, .step-card, .card-face",
        );
    ...
    inspectCards() {
        ...
        cards.forEach((card, index) => {
            ...
            if (image) {
                ...
                this.log(
                    `    Image loaded: ${image.complete && image.naturalWidth > 0}`,
                );
            } else {
    ...
    checkImages() {
        ...
        images.forEach((img, index) => {
            ...
            if (containerRect) {
                this.log(
                    `    Container: ${containerRect.width.toFixed(1)}x${containerRect.height.toFixed(1)}`,
                );
            }
    ...
    enableElementHighlighting() {
        ...
        const handleMouseOver = (e) => {
            if (
                e.target !== this.debugElement &&
                !this.debugElement.contains(e.target)
            ) {
