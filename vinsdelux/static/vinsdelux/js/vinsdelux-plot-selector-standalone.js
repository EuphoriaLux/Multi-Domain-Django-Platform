...
class StandalonePlotSelector {
    ...
    async loadPlots() {
        try {
            ...
            const isVinsDeluxDomain =
                hostname === "vinsdelux.com" || hostname === "www.vinsdelux.com";
            ...
            this.plots = data.adoption_plans.map((plan) => ({
                ...
                description:
                    plan.description || "Exceptional wine adoption opportunity.",
            ...
            this.showWarningMessage(
                "Unable to load live data. Showing sample adoption plans for demonstration.",
            );
    ...
    buildFeaturesList(plan) {
        ...
        if (
            plan.producer.vineyard_features &&
            plan.producer.vineyard_features.length > 0
        ) {
    ...
    getPrimaryImage(plan) {
        ...
        if (plan.images && plan.images.length > 0) {
            ...
            const primaryImg = plan.images.find(
                (img) => img.is_primary && !img.is_default,
            );
        ...
        const vineyardDefaultsUrl =
            document.body.dataset.vineyardDefaultsUrl ||
    ...
    getRotatingImages(plot) {
        ...
        if (plot.images && plot.images.length > 0) {
            ...
            const uploadedImages = plot.images.filter(
                (img) => !img.is_default && img.url,
            );
    ...
    showNoDataMessage() {
        ...
        if (container) {
            container.innerHTML =
                '<div class="no-data-message">No adoption plans available at this time.</div>';
        }
    }
    ...
    showWarningMessage(message) {
        ...
        if (container) {
            ...
            warningDiv.style.cssText =
                "padding: 15px; background: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; color: #856404;";
        ...
        return [
            {
                ...
                description:
                    "Experience the legendary Château Margaux with this exclusive adoption plan.",
                x: 30,
                y: 40,
                includesVisit: true,
            ...
            },
            {
                ...
                description:
                    "Discover the magic of Tuscan winemaking with this premium adoption.",
                x: 70,
                y: 60,
                includesVisit: true,
    ...
    selectPlot(plotId) {
        ...
        const selectedCard = document.querySelector(
            `.plot-card[data-plot-id="${plotId}"]`,
        );
        const selectedMarker = document.querySelector(
            `.plot-marker[data-plot-id="${plotId}"]`,
        );
    ...
    showPlotDetails(plot) {
        ...
        if (deliveriesEl)
            deliveriesEl.textContent = `${plot.coffretsPerYear || 4} coffrets`;
        ...
        if (coffretContainer) {
            ...
            coffretContainer.querySelectorAll(".coffret-option").forEach((option) => {
                option.addEventListener("click", (e) => {
                    ...
                    coffretContainer
                        .querySelectorAll(".coffret-option")
                        .forEach((opt) => {
                    ...
                    this.selectedCoffret =
                        coffretOptions[parseInt(option.dataset.coffretId)];
                });
            });
    ...
    getCoffretOptionsForPlot(plot) {
        ...
        if (plot.availableCoffrets && plot.availableCoffrets.length > 0) {
            return plot.availableCoffrets.map((coffret) => ({
                ...
                description:
                    coffret.description ||
                    `${coffret.bottles_count || 6} selected wines`,
            ...
            return [
                {
                    ...
                    description:
                        plot.coffret.description ||
                        `${plot.coffret.bottles_count || 6} carefully selected wines`,
    ...
    confirmSelection() {
        ...
        this.showNotification(
            `Plot "${this.selectedPlot.name}" selected successfully!`,
        );
