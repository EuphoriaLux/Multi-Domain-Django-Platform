...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not producers:
            self.stdout.write(
                self.style.ERROR(
                    "No producers found. Please create some producers first."
                )
            )
            return
        ...
        elevations = ["320m", "350m", "380m", "420m", "450m", "480m", "520m", "550m"]
        ...
        for i in range(count):
            ...
            plot_name = (
                f"{producer.name} - {template['name_template'].format('')}".replace(
                    " - ", " "
                ).strip()
            )
            if i % 3 == 0:
            ...
            plot = VdlPlot.objects.create(
                ...
                is_premium=random.choices([True, False], weights=[20, 80])[
                    0
                ],  # 20% premium
            )
            ...
            if adoption_plans:
                ...
                selected_plans = random.sample(
                    adoption_plans, min(random.randint(1, 3), len(adoption_plans))
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully created {len(created_plots)} sample plots:"
            )
        )

        for plot in created_plots:
            status_color = (
                self.style.SUCCESS if plot.is_available else self.style.WARNING
            )
        ...
        self.stdout.write(
            f'  Producers with plots: {VdlPlot.objects.values("producer").distinct().count()}'
        )

        if adoption_plans:
            self.stdout.write(
                f"  Plots with adoption plans: {VdlPlot.objects.filter(adoption_plans__isnull=False).distinct().count()}"
            )
