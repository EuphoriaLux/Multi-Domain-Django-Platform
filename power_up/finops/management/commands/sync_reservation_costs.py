...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not reservations:
            self.stdout.write(
                self.style.WARNING("No reservations found in FOCUS exports")
            )
            return
        ...
        for res in reservations:
            ...
            if not force_refresh:
                ...
                if existing:
                    hours_since_sync = (
                        timezone.now() - existing.last_synced
                    ).total_seconds() / 3600
                    if hours_since_sync < 24:
                        self.stdout.write(
                            f"   [SKIP] Last synced {hours_since_sync:.1f} hours ago"
                        )
                        skipped_count += 1
                        continue
            ...
            try:
                ...
                if not metadata:
                    self.stdout.write(
                        self.style.ERROR("   [ERROR] Failed to fetch metadata")
                    )
                    error_count += 1
                    continue
                ...
                if not pricing:
                    self.stdout.write(
                        self.style.WARNING(
                            "   [WARN] No pricing found in Retail Prices API"
                        )
                    )
                    error_count += 1
                    continue
    ...
    def get_reservation_metadata(self, reservation_id):
        ...
        try:
            ...
            if not full_id:
                self.stdout.write(
                    f"   [WARN] No FOCUS record found with reservation ID {reservation_id}"
                )
                return None
            ...
            if not access_token:
                self.stdout.write(
                    self.style.WARNING("   [WARN] Failed to get Azure access token")
                )
                return None
            ...
            if response.status_code != 200:
                self.stdout.write(
                    self.style.WARNING(
                        f"   [WARN] Azure API error: {response.status_code} - {response.text[:200]}"
                    )
                )
                return None
            ...
            term_years = (
                int(term[1]) if term.startswith("P") and term.endswith("Y") else 3
            )
            ...
            parts = full_id.split("/")
            order_index = (
                parts.index("reservationOrders") if "reservationOrders" in parts else -1
            )
            reservation_order_id = (
                parts[order_index + 1]
                if order_index >= 0 and order_index + 1 < len(parts)
                else ""
            )
            ...
            return {
                ...
                "service_name": self.map_resource_type_to_service(
                    props.get("reservedResourceType", "")
                ),
            ...
            self.stdout.write(
                self.style.ERROR(f"   [ERROR] Metadata fetch failed: {str(e)}")
            )
            return None
    ...
    def get_reservation_pricing(self, service_name, region, term_years, focus_data):
        ...
        try:
            ...
            meter_subcategory = focus_data.get(
                "x_SkuMeterSubcategory"
            )  # e.g., "Azure App Service Premium v3 Plan - Linux"
            term_months = int(
                focus_data.get("x_SkuTerm", term_years * 12)
            )  # e.g., "36"
            ...
            self.stdout.write(
                f"   Matching: {meter_subcategory} | SKU: {focus_sku} | {term_months} months"
            )
            ...
            params = {"$filter": filter_query, "currencyCode": "EUR"}
            ...
            if not items:
                self.stdout.write(
                    self.style.WARNING(
                        f"   [WARN] No reservations found for {service_name} in {region}"
                    )
                )
                return None
            ...
            for item in items:
                ...
                if product_name == meter_subcategory and focus_sku:
                    ...
                    sku_match = api_sku.lower().replace(
                        " ", ""
                    ) == focus_sku.lower().replace(" ", "")

                    if sku_match and (
                        reservation_term == term_str_singular
                        or reservation_term == term_str_plural
                    ):
                        self.stdout.write(
                            f"   [MATCH] Exact match: {product_name} | SKU: {api_sku}"
                        )
                        return {
            ...
            for item in items:
                ...
                if meter_subcategory and product_name in meter_subcategory:
                    if (
                        ...
                        self.stdout.write(
                            f"   [MATCH] Found partial match: {product_name}"
                        )
                        return {
                            "total_cost": item["unitPrice"],
                            "unit_price": item["unitPrice"],
            ...
            for item in items:
                ...
                if (
                    ...
                    if "linux" in product_name:
                        self.stdout.write(
                            f'   [WARN] Fallback match (term + Linux): {item["productName"]}'
                        )
                        return {
                            "total_cost": item["unitPrice"],
                            "unit_price": item["unitPrice"],
            ...
            for item in items:
                ...
                if (
                    ...
                    self.stdout.write(
                        f'   [WARN] Fallback match (term only): {item["productName"]}'
                    )
                    return {
                        "total_cost": item["unitPrice"],
                        "unit_price": item["unitPrice"],
            ...
            self.stdout.write(
                self.style.WARNING("   [WARN] No matching reservation found")
            )
            return None

        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"   [ERROR] Pricing API failed: {str(e)}")
            )
            return None
    ...
    def get_azure_access_token(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "   [WARN] azure-identity not installed, falling back to subprocess"
                )
            )
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"   [WARN] Azure CLI error: {result.stderr[:200]}"
                        )
                    )
                    return None
                ...
                self.stdout.write(
                    self.style.WARNING(
                        '   [WARN] Azure CLI not found - install azure-identity or run "az login"'
                    )
                )
                return None
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(f"   [WARN] Token fetch failed: {str(e)}")
                )
                return None

        except Exception as e:
            self.stdout.write(
                self.style.WARNING(f"   [WARN] Authentication failed: {str(e)}")
            )
            return None
