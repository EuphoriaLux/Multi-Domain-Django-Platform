...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--currency", type=str, default="EUR", help="Currency code")
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"Reservation not found: {reservation_id}")
            )
            return
        except ReservationCost.MultipleObjectsReturned:
            self.stdout.write(
                self.style.ERROR(
                    f"Multiple reservations found with ID: {reservation_id}"
                )
            )
            return
