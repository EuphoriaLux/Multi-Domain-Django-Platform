...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--dry-run", action="store_true", help="Detect but do not save anomalies"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(
                f"Detecting anomalies for last {days_back} days ({currency})..."
            )
        )

        # Run detection
        anomalies = CostAnomalyDetector.detect_daily_anomalies(
            currency=currency, days_back=days_back
        )
        ...
        for anomaly in sorted(
            anomalies, key=lambda x: abs(float(x.deviation_percent)), reverse=True
        )[:5]:
            self.stdout.write(
        ...
        try:
            CostAnomaly.objects.bulk_create(
                anomalies, ignore_conflicts=True, batch_size=100
            )
            self.stdout.write(
