...
def cost_summary(request):
    ...
    top_subscriptions = (
        records.values("sub_account_name")
        .annotate(cost=Sum("billed_cost"))
        .order_by("-cost")[:5]
    )

    top_services = (
        records.values("service_name")
        .annotate(cost=Sum("billed_cost"))
        .order_by("-cost")[:10]
    )

    top_resource_groups = (
        records.annotate(rg_lower=Lower("resource_group_name"))
        .values("rg_lower")
        .annotate(cost=Sum("billed_cost"))
        .order_by("-cost")[:10]
    )
    ...
    daily_costs = (
        records.annotate(usage_date=TruncDate("charge_period_start"))
        .values("usage_date")
        .annotate(cost=Sum("billed_cost"))
        .order_by("usage_date")
    )
...
def costs_by_subscription(request):
    ...
    date_info = resolve_date_range(
        request.query_params,
        queryset=CostAggregation.objects.filter(
            dimension_type="subscription", currency=currency
        ),
...
def costs_by_service(request):
    ...
    services = (
        CostRecord.objects.filter(**filters)
        .values("service_name")
        .annotate(
            total_cost=Sum("billed_cost"),
...
def costs_by_resource_group(request):
    ...
    resource_groups = (
        CostRecord.objects.filter(**filters)
        .annotate(rg_lower=Lower("resource_group_name"))
        .values("rg_lower")
        .annotate(
            total_cost=Sum("billed_cost"),
            record_count=Count("id"),
    ...
    return Response(
        {
            ...
            "resource_groups": [
                {
                    "resource_group_name": rg["rg_lower"],
...
def cost_trend(request):
    ...
    daily_costs = (
        CostRecord.objects.filter(**base_filters)
        .annotate(usage_date=TruncDate("charge_period_start"))
...
def export_status(request):
    ...
    failed_imports = CostExport.objects.filter(import_status="failed").order_by(
        "-import_started_at"
    )[:5]
    ...
    return Response(
        {
            "summary": {
                ...
                "success_rate": (
                    round(completed_exports / total_exports * 100, 2)
                    if total_exports > 0
                    else 0
                ),
            },
...
def cost_anomalies(request):
    ...
    return Response({"success": True, "count": len(data), "anomalies": data})
...
def acknowledge_anomaly(request, anomaly_id):
    ...
    try:
        ...
        return Response(
            {"success": True, "message": "Anomaly acknowledged successfully"}
        )
...
def export_costs_csv(request):
    ...
    if export_format == "daily_summary":
        writer.writerow(
            ["Date", "Total Cost", "Usage Cost", "Purchase Cost", "Record Count"]
        )
        daily_costs = (
            CostRecord.objects.filter(**filters)
            .annotate(usage_date=TruncDate("charge_period_start"))
        ...
        total = (
            CostRecord.objects.filter(**filters).aggregate(total=Sum("billed_cost"))[
                "total"
            ]
            or 1
        )
