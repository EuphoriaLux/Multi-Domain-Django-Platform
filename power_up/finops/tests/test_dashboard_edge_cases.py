...
def staff_user(db):
    ...
    return User.objects.create_user(
        username="staff", email="staff@powerup.lu", password="testpass", is_staff=True
    )
...
class TestFinOpsDashboardEdgeCases:
    ...
    def test_dashboard_365_days_all_charges(self, client_logged_in):
        ...
        response = client_logged_in.get(
            "/finops/?days=365&charge_type=all&subscription=&service="
        )
        assert response.status_code == 200
        assert b"Total Cost" in response.content
    ...
    def test_dashboard_default_usage_filter(self, client_logged_in):
        ...
        assert (
            b"Showing" in response.content and b"Usage costs only" in response.content
        )
    ...
    def test_concurrent_filters(self, client_logged_in):
        ...
        response = client_logged_in.get(
            "/finops/?days=365&charge_type=all&subscription=test&service=storage"
        )
        assert response.status_code == 200
