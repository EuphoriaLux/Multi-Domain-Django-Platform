...
def dashboard(request):
    ...
    ).filter(usage_date__gte=start_date, usage_date__lte=end_date)
    ...
    cost_aggregates = base_queryset.aggregate(
        total_effective=Sum("effective_cost"), total_list=Sum("list_cost")
    )
    ...
    if charge_type_filter != "reserved":
        for reservation in active_reservations:
            if reservation.is_active(start_date) or reservation.is_active(end_date):
                period_cost = reservation.get_amortized_cost_for_period(
                    start_date, end_date
                )
                reservation_cost += Decimal(str(period_cost))
                estimated_payg = period_cost / 0.45
                reservation_savings_estimate += Decimal(
                    str(estimated_payg - period_cost)
                )
    ...
    mtd_ytd_queryset = CostRecord.objects.annotate(
        usage_date=TruncDate("charge_period_start")
    ).filter(usage_date__gte=year_start, usage_date__lte=end_date)
    if charge_type_filter and charge_type_filter != "all":
        if charge_type_filter == "payg":
            mtd_ytd_queryset = mtd_ytd_queryset.filter(
                extended_data__PricingCategory="Standard"
            )
        elif charge_type_filter == "reserved":
            mtd_ytd_queryset = mtd_ytd_queryset.filter(
    ...
    latest_export = (
        CostExport.objects.filter(import_status="completed")
        .order_by("-billing_period_end")
        .first()
    )

    top_subscriptions = (
        base_queryset.values("sub_account_name")
        .annotate(cost=Sum("billed_cost"))
        .order_by("-cost")[:5]
    )

    top_services = (
        base_queryset.values("service_name")
        .annotate(cost=Sum("billed_cost"))
        .order_by("-cost")[:10]
    )
    ...
    if all_subscriptions is None:
        all_subscriptions = list(
            CostRecord.objects.values_list("sub_account_name", flat=True)
            .distinct()
            .order_by("sub_account_name")
        )
    ...
    if all_services is None:
        all_services = list(
            CostRecord.objects.values_list("service_name", flat=True)
            .distinct()
    ...
    tag_keys = []
    tag_records = (
        CostRecord.objects.exclude(tags__isnull=True)
        .exclude(tags={})
        .values_list("tags", flat=True)[:100]
    )
    seen_keys = set()
    ...
    try:
        ...
        for budget in CostBudget.objects.filter(is_active=True):
            ...
            utilization = (
                (float(spent) / float(budget.monthly_budget) * 100)
...
def subscription_view(request):
    ...
    subscriptions = (
        CostRecord.objects.annotate(usage_date=TruncDate("charge_period_start"))
        .filter(usage_date__gte=start_date, usage_date__lte=end_date)
        .values("sub_account_name")
        .annotate(total_cost=Sum("billed_cost"), record_count=Count("id"))
        .order_by("-total_cost")
    )
...
def service_breakdown(request):
    ...
    services = (
        base_queryset.values("service_name", "service_category")
        .annotate(total_cost=Sum("billed_cost"), record_count=Count("id"))
        .order_by("-total_cost")
    )
...
def resource_explorer(request):
    ...
    resources = (
        base_queryset.values("resource_name")
        .annotate(
    ...
    available_subscriptions = (
        filter_base.values_list("sub_account_name", flat=True)
        .distinct()
        .order_by("sub_account_name")
    )

    available_resource_groups = (
        filter_base.annotate(rg_lower=Lower("resource_group_name"))
        .values_list("rg_lower", flat=True)
        .distinct()
        .order_by("rg_lower")
    )
...
def trigger_import(request):
    ...
    if request.method == "POST":
        return JsonResponse(
            {"status": "info", "message": "Run: python manage.py import_cost_data"}
        )
...
def anomalies_view(request):
    ...
    context = {
        ...
        "anomalies": anomalies,
        "critical_count": CostAnomaly.objects.filter(
            severity="critical", is_acknowledged=False
        ).count(),
        "high_count": CostAnomaly.objects.filter(
            severity="high", is_acknowledged=False
        ).count(),
        "medium_count": CostAnomaly.objects.filter(
            severity="medium", is_acknowledged=False
        ).count(),
        "low_count": CostAnomaly.objects.filter(
...
def forecast_view(request):
    ...
    forecasts = CostForecast.objects.filter(
        dimension_type="overall", dimension_value="Total"
    ...
    if avg_historical > 0:
        pct_change = (
            (float(avg_forecast) - float(avg_historical)) / float(avg_historical)
        ) * 100
    else:
        pct_change = 0
...
def comparison_view(request):
    ...
    chart_services = sorted(
        comparison_data, key=lambda x: x["month1_cost"] + x["month2_cost"], reverse=True
    )[:10]
...
def resource_group_view(request):
    ...
    resource_groups = (
        base_queryset.annotate(rg_lower=Lower("resource_group_name"))
        .values("rg_lower")
        .annotate(total_cost=Sum("billed_cost"), record_count=Count("id"))
        .order_by("-total_cost")
    )
