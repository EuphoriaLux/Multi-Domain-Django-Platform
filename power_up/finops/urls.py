# power_up/finops/urls.py
"""
URL configuration for FinOps Hub (power_up app)

These URL patterns are included from urls_power_up.py under the 'finops/' prefix.
"""
...
router.register(
    r"aggregations", api_views.CostAggregationViewSet, basename="aggregation"
)
...
urlpatterns = [
    ...
    path("resource-groups/", views.resource_group_view, name="resource_groups"),
    path("import/", views.trigger_import, name="import"),
    path(
        "import/<int:export_id>/update-subscription/",
        views.update_subscription_id,
    ...
    path(
        ...
        api_views.costs_by_subscription,
        name="api_costs_by_subscription",
    ),
    path(
        "api/costs/by-service/", api_views.costs_by_service, name="api_costs_by_service"
    ),
    path(
        "api/costs/by-resource-group/",
        api_views.costs_by_resource_group,
        name="api_costs_by_resource_group",
    ),
    ...
    path(
        "api/anomalies/<int:anomaly_id>/acknowledge/",
        api_views.acknowledge_anomaly,
        name="api_acknowledge_anomaly",
    ),
