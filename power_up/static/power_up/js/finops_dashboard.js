...
(function () {
    ...
    function initDashboard(config) {
        ...
        window.toggleChartView = function (view) {
            ...
            if (dailyBtn)
                dailyBtn.className = view === "daily" ? activeClass : inactiveClass;
            if (weeklyBtn)
                weeklyBtn.className = view === "weekly" ? activeClass : inactiveClass;
        ...
        function aggregateWeekly(dailyCosts) {
            ...
            return Object.values(weeklyData).sort(function (a, b) {
                return new Date(a.usage_date) - new Date(b.usage_date);
            });
        }
        ...
        function renderCostTrendChart(dailyCosts, view) {
            ...
            var labels = data.map(function (d) {
                return formatDateLabel(d.usage_date, view);
            });
            var usageCosts = data.map(function (d) {
                return parseFloat(d.usage_cost || 0);
            });
            var purchaseCosts = data.map(function (d) {
                return parseFloat(d.purchase_cost || 0);
            });
        ...
        function renderSubscriptionChart(subscriptions) {
            ...
            var ctx = canvas.getContext("2d");
            var labels = subscriptions.map(function (s) {
                return s.sub_account_name || "Unknown";
            });
            var costs = subscriptions.map(function (s) {
                return parseFloat(s.cost);
            });
            var colors = [
                "rgba(239,68,68,0.9)",
                "rgba(59,130,246,0.9)",
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: function (ctx) {
                                    var total = ctx.dataset.data.reduce(function (
                                        a,
                                        b,
                                    ) {
                                        return a + b;
        ...
        function initDiscountSlider(originalTotal) {
            ...
            function applyDiscount(pct) {
                ...
                document.querySelectorAll(".service-row").forEach(function (row) {
                    ...
                    var isMicrosoft =
                        name.toLowerCase().indexOf("azure") >= 0 ||
                        name.toLowerCase().indexOf("microsoft") >= 0;
                    var discounted = orig;
                        ...
                        costCell.innerHTML =
                            isMicrosoft && pct > 0
                                ? '<span class="text-green-600">' +
                                  ...
                                  '</span> <small class="text-gray-400">(-' +
                                  pct +
                                  "%)</small>"
                                : discounted.toFixed(2);
                    }
                ...
                savingsEl.innerHTML =
                    '<span class="text-sm text-gray-500">Savings: </span><strong class="text-green-600">' +
                    savings.toFixed(2) +
                    '</strong><br><small class="text-gray-500">New Total: ' +
                    newTotal.toFixed(2) +
                    "</small>";
            }
        }
    ...
    function initSubscriptionView(config) {
        ...
        var labels = config.subscriptions.map(function (s) {
            return s.sub_account_name || "Unknown";
        });
        var costs = config.subscriptions.map(function (s) {
            return parseFloat(s.total_cost);
        });
        var colors = [
            "rgba(239,68,68,0.9)",
            "rgba(59,130,246,0.9)",
    ...
    function initServiceBreakdown(config) {
        ...
        new Chart(canvas, {
            type: "bar",
            data: {
                labels: config.services.map(function (s) {
                    return s.service_name;
                }),
                datasets: [
                    {
                        label: "Cost (EUR)",
                        data: config.services.map(function (s) {
                            return parseFloat(s.total_cost);
                        }),
    ...
    function initForecast(config) {
        ...
        var historicalLabels = config.historical.map(function (h) {
            return h.date;
        });
        var historicalCosts = config.historical.map(function (h) {
            return h.cost;
        });

        var forecastLabels = config.forecasts.map(function (f) {
            return f.date;
        });
        var forecastCosts = config.forecasts.map(function (f) {
            return f.cost;
        });
        var forecastUpper = config.forecasts.map(function (f) {
            return f.upper;
        });
        var forecastLower = config.forecasts.map(function (f) {
            return f.lower;
        });
        ...
        var historicalDataset = historicalCosts.concat(
            forecastCosts.map(function () {
                return null;
            }),
        );
        var forecastDataset = historicalCosts
            .map(function () {
                return null;
            })
            .concat(forecastCosts);
        var upperDataset = historicalCosts
            .map(function () {
                return null;
            })
            .concat(forecastUpper);
        var lowerDataset = historicalCosts
            .map(function () {
                return null;
            })
            .concat(forecastLower);

        // Bridge: connect last historical point to first forecast point
        if (historicalCosts.length > 0 && forecastCosts.length > 0) {
            forecastDataset[historicalCosts.length - 1] =
                historicalCosts[historicalCosts.length - 1];
            upperDataset[historicalCosts.length - 1] =
                historicalCosts[historicalCosts.length - 1];
            lowerDataset[historicalCosts.length - 1] =
                historicalCosts[historicalCosts.length - 1];
        }
    ...
    function initComparison(config) {
        ...
        var labels = config.services.map(function (s) {
            return s.service_name;
        });
        var month1Data = config.services.map(function (s) {
            return s.month1_cost;
        });
        var month2Data = config.services.map(function (s) {
            return s.month2_cost;
        });
        ...
        new Chart(canvas, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: config.month1Label,
                        data: month1Data,
                        backgroundColor: "rgba(59,130,246,0.8)",
                    },
                    {
                        label: config.month2Label,
                        data: month2Data,
    ...
    function initResourceGroups(config) {
        if (!config || !config.resourceGroups || config.resourceGroups.length === 0)
            return;

        var canvas = document.getElementById("resourceGroupChart");
        if (!canvas) return;

        var labels = config.resourceGroups.map(function (rg) {
            return rg.resource_group_name || "Unknown";
        });
        var costs = config.resourceGroups.map(function (rg) {
            return parseFloat(rg.total_cost);
        });
