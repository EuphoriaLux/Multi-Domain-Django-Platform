...
- **The DB login `crush_analytics_ro` holds only the column grants in `analytics_readonly.GRANTS`.** `manage.py setup_analytics_role` applies them, and adding a column there is a privacy decision. It is also a deploy step: re-run the command after the swap, or the API 503s on the missing grant.
  - `audit_role()` checks what the login can *effectively* do, including anything granted through PUBLIC. The setup command fails on any excess or missing grant, and the service refuses to serve (503) on either, re-checked every 10 minutes.
  - `production.py` accepts no other login name.
...
- **Member-level rows (`members`, `event_detail`) carry gender, age band and canton generalized over the whole real-member population** (`_generalized_quasi_identifiers`), and member filters match those generalized values. The event tools' `seat_holders_by_gender` counts the same generalized genders. Otherwise a filtered `members` call would list a cell that `demographics` suppresses. A `members` result with 1–4 matches after all filters returns nothing, but that is a guardrail, not a guarantee. The privacy model is *pseudonymized, not anonymous* (spec decision D2): only the demographic quasi-identifiers are k-anonymized. The other row fields (status, LuxID, activity, dates) are the analytic payload and stay visible per pseudonym by design. Real members always need a `CrushProfile` (`_with_profile`): profile-less event guests count as excluded accounts.
