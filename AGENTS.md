...
- **The DB login `crush_analytics_ro` holds only the column grants in `analytics_readonly.GRANTS`.** `manage.py setup_analytics_role` applies them, and adding a column there is a privacy decision.
  - `audit_role()` checks what the login can *effectively* do, including anything granted through PUBLIC. The setup command fails on any excess, and the service refuses to serve (503) on any excess, re-checked every 10 minutes.
