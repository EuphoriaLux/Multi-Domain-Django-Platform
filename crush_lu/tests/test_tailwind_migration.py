...
class TailwindMigrationTests(TestCase):
    ...
    BOOTSTRAP_PATTERNS = [
        ...
        (
            r'(?<!-)btn-primary\b(?!.*class=["\'][^"\']*btn-crush)',
            "Bootstrap btn-primary (use btn-crush-primary)",
        ),
        (
            r'(?<!-)btn-secondary\b(?!.*class=["\'][^"\']*btn-crush)',
    ...
    def test_no_bootstrap_component_classes(self):
        ...
        for template_path in self.get_all_templates():
            ...
            for pattern, description in self.BOOTSTRAP_PATTERNS:
                matches = re.findall(pattern, content)
                if matches:
                    violations.append(f"{template_path.name}: {description}")
    ...
    def test_htmx_loaded_in_base(self):
        ...
        self.assertTrue("htmx" in content.lower(), "HTMX script not found in base.html")
    ...
    def test_htmx_csrf_configured(self):
        ...
        has_csrf_config = (
            "hx-headers" in content
            or "htmx:configRequest" in content
            or "X-CSRFToken" in content
            or "htmx-csrf.js" in content  # External CSP-compliant script
        )

        self.assertTrue(
            has_csrf_config, "HTMX CSRF configuration not found in base.html"
        )
    ...
    def test_tailwind_css_loaded(self):
        ...
        self.assertTrue(
            "tailwind" in content.lower(), "Tailwind CSS not found in base.html"
        )
...
class GridMigrationTests(TestCase):
    ...
    def test_tailwind_grid_used_in_migrated(self):
        ...
        for template_name in self.MIGRATED_TEMPLATES:
            ...
            found_grid = any(
                re.search(pattern, content) for pattern in tailwind_grid_patterns
            )

            self.assertTrue(
                found_grid, f"{template_name}: No Tailwind grid classes found"
            )
...
class InlineStyleExtractionTests(TestCase):
    ...
    def test_no_inline_styles_in_migrated_templates(self):
        ...
        for template_name in self.STYLE_FREE_TEMPLATES:
            ...
            real_styles = [
                m for m in style_matches if m.strip() and not m.strip().startswith("{#")
            ]

            if real_styles:
                violations.append(f"{template_name}: Contains inline <style> blocks")
