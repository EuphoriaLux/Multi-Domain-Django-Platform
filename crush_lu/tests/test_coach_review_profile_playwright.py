...
def pytest_runtest_makereport(item, call):
    ...
    if rep.when == "call" and rep.failed:
        ...
        if "page" in item.funcargs or "authenticated_coach_page" in item.funcargs:
            page = item.funcargs.get("authenticated_coach_page") or item.funcargs.get(
                "page"
            )
            if page:
                ...
                screenshot_path = os.path.join(
                    screenshot_dir, f"{test_name}_failure.png"
                )
...
def linkedin_signup_user(transactional_db):
    ...
    social_account = SocialAccount.objects.create(
        user=user, provider="linkedin_oauth2", uid="linkedin123456"
    )
...
@pytest.fixture
def authenticated_coach_page(
    page, live_server_url, coach_user_with_permissions, transactional_db
):
...
def click_tab(page, tab_name):
    ...
    button = (
        page.locator(".flex-col.sm\\:flex-row button").filter(has_text=tab_name).first
    )
...
class TestProfileSummaryCardVisibility:
    ...
    def test_summary_hidden_on_profile_tab(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_summary_visible_on_screening_tab(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_summary_visible_on_decision_tab(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_summary_toggles_with_tab_switches(
        self, authenticated_coach_page, pending_profile_submission
    ):
...
class TestAccountMetadataDisplay:
    ...
    def test_account_age_displayed(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_email_signup_displayed(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_linkedin_signup_displayed(
        self,
        authenticated_coach_page,
        coach_user_with_permissions,
        linkedin_signup_user,
    ):
    ...
    def test_phone_verification_displayed(
        self, authenticated_coach_page, phone_verified_profile
    ):
    ...
    def test_last_activity_displayed(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_metadata_section_has_divider(
        self, authenticated_coach_page, pending_profile_submission
    ):
...
class TestDifferentAccountTypes:
    ...
    def test_recent_account_display(
        self, authenticated_coach_page, coach_user_with_permissions, email_signup_user
    ):
    ...
    def test_older_account_display(
        self, authenticated_coach_page, coach_user_with_permissions, old_account_user
    ):
    ...
    def test_unverified_phone_no_verification_badge(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_user_never_logged_in(
        self, authenticated_coach_page, coach_user_with_permissions
    ):
...
class TestTabSwitching:
    ...
    def test_default_tab_is_profile(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_switch_to_screening_tab(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_switch_to_decision_tab(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_active_tab_styling(
        ...
        profile_tab_button = (
            page.locator(".flex-col.sm\\:flex-row button")
            .filter(has_text="Profile Overview")
            .first
        )
        ...
        screening_tab_button = (
            page.locator(".flex-col.sm\\:flex-row button")
            .filter(has_text="Screening Call")
            .first
        )
        classes = screening_tab_button.get_attribute("class")
        assert "bg-white" in classes or "text-purple" in classes

    def test_view_full_profile_button(
        self, authenticated_coach_page, pending_profile_submission
    ):
...
class TestCompleteScreeningWorkflow:
    ...
    def test_screening_call_warning_indicator(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_schedule_screening_call(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_complete_screening_form(
        self, authenticated_coach_page, pending_profile_submission
    ):
...
class TestDecisionSubmission:
    ...
    def test_decision_form_visible(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_feedback_to_user_field(
        self, authenticated_coach_page, pending_profile_submission
    ):
...
class TestVisualElements:
    ...
    def test_profile_photos_displayed(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_gradient_background_on_summary(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_icons_render(self, authenticated_coach_page, pending_profile_submission):
        ...
        assert (
            "🗓️" in summary_card.text_content() or "📧" in summary_card.text_content()
        )

    def test_green_color_for_verified(
        self, authenticated_coach_page, phone_verified_profile
    ):
        ...
        verified_text = page.locator(
            '[x-show="showProfileSummary"] p.text-green-600:has-text("Phone verified")'
        ).first
...
class TestResponsiveDesign:
    ...
    def test_mobile_layout(self, authenticated_coach_page, pending_profile_submission):
        ...
        tab_navigation = (
            page.locator(".flex-col.sm\\:flex-row")
            .filter(has_text="Profile Overview")
            .first
        )
...
class TestEdgeCases:
    ...
    def test_missing_social_account(
        self, authenticated_coach_page, pending_profile_submission
    ):
    ...
    def test_back_to_dashboard_button(
        self, authenticated_coach_page, pending_profile_submission
    ):
