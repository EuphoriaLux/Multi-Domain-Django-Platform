...
class TestScreeningCallView(TestCase):
    ...
    def test_mark_call_complete_saves_checklist_data(self):
        ...
        url = reverse(
            "crush_lu:coach_mark_review_call_complete", args=[self.submission.id]
        )
        ...
        self.assertEqual(
            self.submission.review_call_notes, "Great call, user is ready."
        )
        ...
        self.assertEqual(
            self.submission.review_call_checklist["residence_notes"],
    ...
    def test_mark_call_complete_with_empty_checklist(self):
        ...
        response = self.client.post(
            url,
            {"call_notes": "Quick call.", "checklist_data": ""},
            HTTP_HX_REQUEST="true",
        )
    ...
    def test_mark_call_complete_with_invalid_json(self):
        ...
        response = self.client.post(
            url,
            {"call_notes": "Call done.", "checklist_data": "not valid json {"},
            HTTP_HX_REQUEST="true",
        )
    ...
    def test_htmx_response_shows_completed_state(self):
        ...
        response = self.client.post(
            url,
            {"call_notes": "Done.", "checklist_data": json.dumps(checklist_data)},
            HTTP_HX_REQUEST="true",
        )
    ...
    def test_non_htmx_request_redirects(self):
        ...
        response = self.client.post(
            url, {"call_notes": "Done.", "checklist_data": "{}"}
        )
    ...
    def test_unauthorized_coach_cannot_mark_complete(self):
        ...
        UserDataConsent.objects.filter(user=other_coach_user).update(
            crushlu_consent_given=True
        )
        CrushCoach.objects.create(
            user=other_coach_user, bio="Other coach", is_active=True
        )
        ...
        response = self.client.post(
            url,
            {"call_notes": "Trying to hijack.", "checklist_data": "{}"},
            HTTP_HX_REQUEST="true",
        )
