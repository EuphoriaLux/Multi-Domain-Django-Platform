...
class TestGiftCreationFlow:
    ...
    def test_gift_create_form_renders(
        self, authenticated_sender_page: Page, live_server_url
    ):
        ...
        assert authenticated_sender_page.locator(
            'input[name="recipient_name"]'
        ).is_visible()
        assert authenticated_sender_page.locator(
            'input[name="date_first_met"]'
        ).is_visible()
        assert authenticated_sender_page.locator(
            'input[name="location_first_met"]'
        ).is_visible()
        assert (
            authenticated_sender_page.locator(
                'textarea[name="sender_message"]'
            ).is_visible()
            or authenticated_sender_page.locator(
                'input[name="sender_message"]'
            ).is_visible()
        )

    def test_gift_create_success(
        self, authenticated_sender_page: Page, live_server_url
    ):
        ...
        authenticated_sender_page.fill(
            'input[name="location_first_met"]', "Luxembourg City"
        )

        # Handle sender_message (could be textarea or input)
        message_field = authenticated_sender_page.locator(
            'textarea[name="sender_message"]'
        )
        if message_field.is_visible():
            ...
            authenticated_sender_page.fill(
                'input[name="sender_message"]', "A special journey for you!"
            )
        ...
        assert (
            ...
            or "WOY-" in authenticated_sender_page.url
        )

    def test_gift_success_shows_qr_code(
        self, authenticated_sender_page: Page, live_server_url
    ):
        ...
        authenticated_sender_page.fill(
            'input[name="location_first_met"]', "Test Location"
        )
        ...
        assert (
            "/journey/gift/success/" in authenticated_sender_page.url
            or "WOY-" in page_content
            or "gift" in page_content.lower()
        )

    def test_gift_success_shows_gift_link(
        self, authenticated_sender_page: Page, live_server_url
    ):
        ...
        authenticated_sender_page.fill(
            'input[name="recipient_name"]', "Link Test Person"
        )
        authenticated_sender_page.fill('input[name="date_first_met"]', "2024-01-01")
        authenticated_sender_page.fill(
            'input[name="location_first_met"]', "Test Location"
        ...
        assert (
            "WOY-" in page_content
            or "/journey/gift/" in page_content
            or "/journey/gift/success/" in authenticated_sender_page.url
        )

    def test_gift_list_shows_created_gift(
        self, authenticated_sender_page: Page, live_server_url
    ):
...
class TestGiftLandingPage:
    ...
    def test_gift_landing_accessible_unauthenticated(
        ...
        assert (
            pending_gift.recipient_name in page.content()
            or "journey" in page.content().lower()
            or "gift" in page.content().lower()
        )

    def test_gift_landing_shows_sender_name(
        self, page: Page, live_server_url, pending_gift
    ):
    ...
    def test_gift_landing_shows_recipient_name(
        self, page: Page, live_server_url, pending_gift
    ):
    ...
    def test_gift_landing_shows_signup_link(
        self, page: Page, live_server_url, pending_gift
    ):
        ...
        signup_link = page.locator(
            'a[href*="signup"], a[href*="register"], button:has-text("Sign"), a:has-text("Sign")'
        )
        assert signup_link.count() > 0

    def test_gift_landing_shows_login_link(
        self, page: Page, live_server_url, pending_gift
    ):
        ...
        login_link = page.locator(
            'a[href*="login"], a:has-text("Log in"), a:has-text("Sign in")'
        )
        assert login_link.count() > 0

    def test_expired_gift_shows_expired_page(
        self, page: Page, live_server_url, expired_gift
    ):
        ...
        assert (
            "expired" in content or "no longer" in content or "unavailable" in content
        )

    def test_claimed_gift_shows_claimed_page(
        self, page: Page, live_server_url, claimed_gift
    ):
...
class TestGiftClaimNewUser:
    ...
    def test_signup_from_gift_landing(self, page: Page, live_server_url, pending_gift):
        ...
        # Click signup link
        signup_link = page.locator(
            'a[href*="signup"], a:has-text("Sign up"), button:has-text("Sign up")'
        ).first
        if signup_link.is_visible():
            ...
            assert (
                ...
                or "/accounts/" in page.url
            )

    def test_claim_after_signup_creates_journey(
        self, page: Page, live_server_url, pending_gift, db
    ):
    ...
    def test_redirect_to_journey_after_claim(
        self, authenticated_recipient_page: Page, live_server_url, pending_gift
    ):
        """After claiming, user should be redirected to journey map."""
        # Navigate to claim page
        authenticated_recipient_page.goto(
            f"{live_server_url}/en/journey/gift/{pending_gift.gift_code}/claim/"
        )
        authenticated_recipient_page.wait_for_load_state("networkidle")

        # Dismiss cookie banner if present
        cookie_decline = authenticated_recipient_page.locator(
        ...
        # Click claim button
        claim_button = authenticated_recipient_page.locator(
            'button:has-text("Claim Your Journey")'
        )
...
class TestGiftClaimExistingUser:
    ...
    def test_authenticated_user_redirected_to_claim(
        self, authenticated_recipient_page: Page, live_server_url, pending_gift
    ):
        """Authenticated users visiting gift landing should be redirected to claim page."""
        authenticated_recipient_page.goto(
            f"{live_server_url}/en/journey/gift/{pending_gift.gift_code}/"
        )
        authenticated_recipient_page.wait_for_load_state("networkidle")

        # Should be redirected to claim page
        assert (
            "/claim" in authenticated_recipient_page.url
            or "Claim" in authenticated_recipient_page.content()
        )

    def test_existing_user_can_claim(
        self, authenticated_recipient_page: Page, live_server_url, pending_gift, db
    ):
    ...
    def test_login_from_gift_landing(
        self, page: Page, live_server_url, pending_gift, recipient_user
    ):
        ...
        if login_link.is_visible():
            ...
            assert (
                "/journey/" in page.url or "/gift/" in page.url
            ), f"Expected redirect to journey/gift, but URL is {page.url}"
...
class TestJourneyAfterClaim:
    ...
    def test_journey_map_shows_chapters(
        self, authenticated_recipient_page: Page, live_server_url, pending_gift, db
    ):
    ...
    def test_personalization_in_journey(
        self, authenticated_recipient_page: Page, live_server_url, pending_gift, db
    ):
...
class TestGiftEdgeCases:
    ...
    def test_invalid_gift_code_404(self, page: Page, live_server_url):
        ...
        assert (
            ...
            or "error" in content
            or page.url != f"{live_server_url}/en/journey/gift/INVALID-CODE-12345/"
        )
    ...
    def test_double_claim_prevented(
        ...
        assert (
            ...
            or "/journey/" in authenticated_recipient_page.url
        )

    def test_gift_status_updates_after_claim(
        self, authenticated_recipient_page: Page, live_server_url, pending_gift, db
    ):
    ...
    def test_sender_sees_claimed_status(
        self,
        authenticated_sender_page: Page,
        live_server_url,
        pending_gift,
        recipient_user,
        db,
