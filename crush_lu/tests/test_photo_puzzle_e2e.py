...
def puzzle_user(transactional_db):
    ...
    return {"user": user, "journey": journey, "chapter": chapter, "reward": reward}


@pytest.fixture
def authenticated_puzzle_page(
    page: Page, live_server_url, puzzle_user, transactional_db
):
...
class TestPhotoPuzzleUI:
    ...
    def test_puzzle_page_loads(
        self, authenticated_puzzle_page: Page, live_server_url, puzzle_user
    ):
    ...
    def test_puzzle_shows_16_pieces(
        self, authenticated_puzzle_page: Page, live_server_url, puzzle_user
    ):
    ...
    def test_puzzle_piece_shows_loading_state_on_click(
        self, authenticated_puzzle_page: Page, live_server_url, puzzle_user
    ):
        ...
        first_piece = authenticated_puzzle_page.locator(
        ...
        authenticated_puzzle_page.wait_for_selector(
            ".reveal-puzzle-piece.unlocked", timeout=5000
        )

    def test_points_decrease_after_unlock(
        self, authenticated_puzzle_page: Page, live_server_url, puzzle_user
    ):
    ...
    def test_notification_appears_on_unlock(
        self, authenticated_puzzle_page: Page, live_server_url, puzzle_user
    ):
    ...
    def test_already_unlocked_piece_not_clickable(
        self, authenticated_puzzle_page: Page, live_server_url, puzzle_user
    ):
        ...
        unlocked_piece = authenticated_puzzle_page.locator(
        ...
        assert (
            dialog_shown["count"] == initial_count
        ), "Unlocked piece should not trigger confirm dialog"
...
class TestPhotoPuzzleLoadingState:
    ...
    def test_loading_class_added_during_request(
        self, authenticated_puzzle_page: Page, live_server_url, puzzle_user
    ):
        ...
        try:
            authenticated_puzzle_page.wait_for_selector(
                ".reveal-puzzle-piece.loading", timeout=1000
            )
        ...
        loading_pieces = authenticated_puzzle_page.locator(
            ".reveal-puzzle-piece.loading"
        )
        expect(loading_pieces).to_have_count(0)

    def test_unlock_completes_without_hanging(
        self, authenticated_puzzle_page: Page, live_server_url, puzzle_user
    ):
        ...
        authenticated_puzzle_page.wait_for_selector(
            ".reveal-puzzle-piece.unlocked", timeout=10000
        )
        ...
        assert (
            points_deducted == 50
        ), f"Expected 50 points deducted, got {points_deducted}"
