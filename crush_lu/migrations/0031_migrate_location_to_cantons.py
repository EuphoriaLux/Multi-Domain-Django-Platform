...
def migrate_locations_forward(apps, schema_editor):
    ...
    print(
        f"Migration complete: {migrated_count} profiles migrated, {cleared_count} cleared (need re-selection)"
    )
...
def migrate_locations_reverse(apps, schema_editor):
    ...
    print(
        f"Rollback complete: {reverted_count} profiles reverted to city-based locations"
    )
