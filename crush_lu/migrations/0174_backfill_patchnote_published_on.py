...
A few notes on the currently-live 'v1-8-crush-connect' release are known to
have actually gone out later than the release's released_on (the release
window stayed open for weeks and the ingest endpoint was, until this change,
always stamping *some* date onto release.released_on on every append —
whatever the caller sent, or today when the caller omitted it per the
then-documented routine instructions — so the field drifted away from the
date any individual older note actually went out). Their real publish dates
are recoverable from the PR merge dates in related_commits, so seed those
explicitly rather than leaving them incorrectly dated.
"""
...
KNOWN_PUBLISH_DATES = {
    ...
    "86df7d8ec4c0b0d411577c83d027cdb0bc266442": "2026-07-01",  # WhatsApp not-on-WhatsApp fallback fix
}
