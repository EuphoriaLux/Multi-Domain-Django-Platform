...
def save_profile_step2(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": "Please complete Step 1 first"}, status=400
        )
    except Exception as e:
...
def upload_photo_draft(request):
    ...
    try:
        ...
        if not photo_file:
            return JsonResponse(
                {"success": False, "error": "No photo file provided"}, status=400
            )
            ...
            return JsonResponse(
                {
                    "success": False,
                    "error": _(
                        "Please select at least one event language. You will only be able to sign up for events in your selected languages."
                    ),
                },
                status=400,
            )
...
def complete_profile_submission(request):
    ...
    try:
        ...
        if missing_fields:
            ...
            messages.error(
                request, _("Please complete all required fields before submitting.")
            )
        ...
        try:
            ...
            messages.error(
                request,
                _(
                    "The selected coach is no longer available. Please choose another coach."
                ),
            )
            return redirect("crush_lu:create_profile")
        ...
        is_same_coach = (
            existing_submission and existing_submission.coach_id == selected_coach.id
        )
        ...
        try:
            ...
            logger.error(
                f"Transaction failed for {request.user.email}: {e}", exc_info=True
            )
            messages.error(
                request,
                _("An error occurred while submitting your profile. Please try again."),
            )
            return redirect("crush_lu:create_profile")
        ...
        return JsonResponse(
            {
                ...
                "has_photos": bool(
                    profile.photo_1 or profile.photo_2 or profile.photo_3
                ),
            }
        )
    except CrushProfile.DoesNotExist:
...
def get_social_photos_api(request):
    ...
    try:
        ...
        return JsonResponse({"error": "Error fetching social photos"}, status=500)
...
def import_social_photo(request):
    ...
    try:
        ...
        if not social_account_id:
            return JsonResponse(
                {"success": False, "error": "Missing social_account_id"}, status=400
            )

        if photo_slot not in [1, 2, 3]:
            return JsonResponse(
                {"success": False, "error": "photo_slot must be 1, 2, or 3"}, status=400
            )
...
def upload_profile_photo(request, slot):
    ...
    try:
        ...
        if created:
            logger.info(
                f"Created new profile for user {request.user.id} during photo upload"
            )
