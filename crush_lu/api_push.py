...
def get_vapid_public_key(request):
    ...
    if len(vapid_key) < 80 or len(vapid_key) > 100:
        logger.error(
            f"VAPID_PUBLIC_KEY has invalid length: {len(vapid_key)} (expected ~87)"
        )
        return JsonResponse(
            {"success": False, "error": "Push notifications misconfigured on server"},
            status=503,
        )
    ...
    return JsonResponse({"success": True, "publicKey": vapid_key})
...
def subscribe_push(request):
    ...
    if fingerprint:
        existing_by_fingerprint = PushSubscription.objects.filter(
            user=request.user, device_fingerprint=fingerprint
        ).first()
    ...
    logger.info(
        f"Push subscription {action} for {request.user.username}{fingerprint_info}"
    )
...
def refresh_subscription(request):
    ...
    try:
        ...
        if not new_subscription_data.get("endpoint"):
            return JsonResponse(
                {"success": False, "message": "New subscription endpoint required"},
                status=400,
            )
        ...
        if old_endpoint:
            ...
            subscription = PushSubscription.objects.filter(
                user=request.user, endpoint=old_endpoint
            ).first()
            ...
            if not subscription and hasattr(request.user, "crush_coach"):
                subscription = CoachPushSubscription.objects.filter(
                    coach=request.user.crush_coach, endpoint=old_endpoint
                ).first()
        ...
        if not subscription:
            ...
            subscription = PushSubscription.objects.filter(
                user=request.user, endpoint=new_endpoint
            ).first()

            if not subscription and hasattr(request.user, "crush_coach"):
                subscription = CoachPushSubscription.objects.filter(
                    coach=request.user.crush_coach, endpoint=new_endpoint
                ).first()
        ...
        if subscription:
            ...
            return JsonResponse(
                {"success": True, "message": "Subscription refreshed successfully"}
            )
        else:
        ...
        return JsonResponse(
            {"success": False, "message": "Server error refreshing subscription"},
            status=500,
        )
...
def validate_subscription(request):
    ...
    try:
        ...
        if not endpoint:
            return JsonResponse(
                {"success": False, "message": "Endpoint required"}, status=400
            )

        # Check if subscription exists and is enabled (try user first, then coach)
        subscription = PushSubscription.objects.filter(
            user=request.user, endpoint=endpoint, enabled=True
        ).first()

        if not subscription and hasattr(request.user, "crush_coach"):
            subscription = CoachPushSubscription.objects.filter(
                coach=request.user.crush_coach, endpoint=endpoint, enabled=True
            ).first()
        ...
        if subscription:
            ...
            return JsonResponse(
                {"success": True, "valid": True, "message": "Subscription is healthy"}
            )
        else:
            return JsonResponse(
                {
    ...
    if deleted_count > 0:
        ...
        try:
            ...
            keep_browser_subscription = (
                CoachPushSubscription.objects.filter(coach=coach).filter(query).exists()
            )
...
def delete_push_subscription(request):
    ...
    if deleted_count > 0:
        logger.info(
            f"Push subscription {subscription_id} deleted for {request.user.username}"
        )
        return JsonResponse(
...
def mark_pwa_user(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "PWA status updated", "isPwaUser": True}
        )

    except Exception as e:
        logger.error(f"Error marking PWA user: {e}")
        return JsonResponse(
            {"success": False, "error": "Failed to update PWA status"}, status=500
        )
...
def get_pwa_status(request):
    ...
    try:
        ...
        return JsonResponse(
            {
                ...
                "lastPwaVisit": (
                    activity.last_pwa_visit.isoformat()
        ...
        return JsonResponse({"success": True, "isPwaUser": False, "lastPwaVisit": None})
...
def run_subscription_health_check(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": "An error occurred during health check"},
            status=500,
        )
