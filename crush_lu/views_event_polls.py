...
def poll_list(request):
    ...
    active_polls = EventPoll.objects.filter(
        is_published=True,
        start_date__lte=now,
        end_date__gte=now,
    ).annotate(total_votes=Count("votes"))

    closed_polls = EventPoll.objects.filter(
        is_published=True,
        end_date__lt=now,
    ...
    return render(
        request,
        "crush_lu/event_polls/poll_list.html",
        {
...
def poll_detail(request, poll_id):
    ...
    user_votes = set(
        EventPollVote.objects.filter(poll=poll, user=request.user).values_list(
            "option_id", flat=True
        )
    )
    ...
    show_results = (
        has_voted or poll.is_closed or (poll.show_results_before_close and not can_vote)
    )

    return render(
        request,
        "crush_lu/event_polls/poll_detail.html",
        {
            "poll": poll,
            "options": options,
            "can_vote": can_vote,
