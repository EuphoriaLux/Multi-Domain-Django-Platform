    ...
    for subscription in subscriptions:
        try:
            ...
            if e.response is not None and e.response.status_code == 410:
                logger.warning(
                    f"Deleting expired coach subscription for {coach.user.username} ({subscription.device_name})"
                )
                subscription.delete()
            else:
...
def send_coach_push_to_subscription(
    subscription, title, body, url="/", tag="coach-notification", icon=None, badge=None
):
    """
    ...
    try:
        ...
        # Handle push errors (expired subscription, etc.)
        logger.warning(
            f"Coach WebPush failed for {subscription.coach.user.username}: {e}"
        )
        ...
        # Catch-all for unexpected errors
        logger.error(
            f"Unexpected error sending coach push to {subscription.coach.user.username}: {e}"
        )
        return {"success": False, "error": str(e)}
...
def notify_coach_new_submission(coach, submission):
    ...
    subscriptions = CoachPushSubscription.objects.filter(
        coach=coach, enabled=True, notify_new_submissions=True
    )

    if not subscriptions.exists():
        logger.info(
            f"Coach {coach.user.username} has new submission notifications disabled"
        )
        return {"success": 0, "failed": 0, "total": 0}
...
def broadcast_new_submission_to_channel(submission):
    """
    Fan-out: notify every active coach that a new profile is in the verification
    channel and can be claimed. Uses a per-submission tag so a later "claimed by"
    broadcast can replace the banner on other coaches' devices.
    """
    subs = CoachPushSubscription.objects.filter(
    ...
    results = []
    tag = f"channel-{submission.id}"
    seen_coaches = set()
    for sub in subs:
        coach = sub.coach
        if coach.id in seen_coaches:
            continue
        seen_coaches.add(coach.id)
        with user_language_context(coach.user):
            title = _("New profile in the verification channel")
            body = _("%(name)s submitted a profile — claim it to review") % {
                "name": user_name
            }
            url = reverse("crush_lu:coach_verification_channel")
        results.append(
            send_coach_push_notification(
                coach=coach,
                title=title,
                body=body,
                url=url,
...
def broadcast_submission_claimed(submission, claimed_by):
    """
    Follow-up broadcast: replace the "new in channel" banner on every other
    coach's device with "claimed by X" so banners stay honest.
    """
    subs = (
        ...
        .exclude(coach=claimed_by)
        .select_related("coach__user")
    )

    claimer_name = claimed_by.user.first_name or claimed_by.user.username
    tag = f"channel-{submission.id}"
    seen_coaches = set()
    for sub in subs:
        coach = sub.coach
        ...
        seen_coaches.add(coach.id)
        with user_language_context(coach.user):
            title = _("Claimed by %(name)s") % {"name": claimer_name}
            body = _("Another coach picked this one up.")
            url = reverse("crush_lu:coach_verification_channel")
        send_coach_push_notification(
            coach=coach,
            title=title,
...
def notify_coach_user_revision(coach, submission):
    ...
    subscriptions = CoachPushSubscription.objects.filter(
        coach=coach, enabled=True, notify_user_responses=True
    )

    if not subscriptions.exists():
        logger.info(
            f"Coach {coach.user.username} has user response notifications disabled"
        )
        return {"success": 0, "failed": 0, "total": 0}
    ...
    with user_language_context(coach.user):
        ...
        body = _("%(name)s updated their profile based on your feedback") % {
            "name": user_name
        }
        url = reverse("crush_lu:coach_review_profile", args=[submission.id])

    return send_coach_push_notification(
        coach=coach, title=title, body=body, url=url, tag=f"revision-{submission.id}"
    )
...
def notify_coach_screening_reminder(coach, submission):
    ...
    if not subscriptions.exists():
        logger.info(
            f"Coach {coach.user.username} has screening reminder notifications disabled"
        )
        return {"success": 0, "failed": 0, "total": 0}
...
def notify_coach_system_alert(coach, title, message, url="/coach/dashboard/"):
    ...
    subscriptions = CoachPushSubscription.objects.filter(
        coach=coach, enabled=True, notify_system_alerts=True
    )

    if not subscriptions.exists():
        logger.info(
            f"Coach {coach.user.username} has system alert notifications disabled"
        )
        return {"success": 0, "failed": 0, "total": 0}
