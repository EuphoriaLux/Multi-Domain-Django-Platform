...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options["skip_sample_data"]:
            ...
            self.stdout.write(
                "  Email: testuser1@crush.lu (through testuser30@crush.lu)"
            )
            self.stdout.write("  Password: testuser2025")
            self.stdout.write("\nCoach credentials:")
            self.stdout.write(
                "  Email: marie@crush.lu, thomas@crush.lu, sophie@crush.lu"
            )
    ...
    def clear_azurite_storage(self):
        ...
        if not connection_string:
            self.stdout.write(
                self.style.WARNING("  No Azurite connection string found")
            )
            return
        ...
        try:
            ...
            for container_name in containers:
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f'  Cleared {len(blobs)} blob(s) from "{container_name}"'
                        )
                    )
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f'  Error with container "{container_name}": {e}'
                        )
                    )

            self.stdout.write(
                self.style.SUCCESS(f"  Total: {total_deleted} blob(s) deleted")
            )
    ...
    def clear_database(self):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"  Database cleared ({sequences_reset} sequences reset)"
            )
        )
    ...
    def create_sample_data(self, profile_count):
        ...
        try:
            call_command("setup_local_dev", profile_count=profile_count, verbosity=1)
        except Exception as e:
