...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Found {journeys.count()} Wonderland journey(s) to update.\n"
        )

        for journey in journeys:
            self.stdout.write(f"\nUpdating journey: {journey.journey_name}")
            self.stdout.write(
                f"  For: {journey.special_experience.first_name} {journey.special_experience.last_name}"
            )
    ...
    def update_chapter(self, chapter, journey, dry_run):
        ...
        self.update_chapter_challenges(
            chapter, chapter_num, content_en, content_de, content_fr, journey, dry_run
        )

        # Update rewards
        self.update_chapter_rewards(
            chapter, content_de, content_fr, first_name, dry_run
        )

    def update_chapter_challenges(
        self, chapter, chapter_num, content_en, content_de, content_fr, journey, dry_run
    ):
        ...
        if chapter_num == 1:
            ...
            self._update_from_challenges_array(
                challenges, content_de, content_fr, dry_run
            )
            ...
            if len(challenges) > 0:
                ...
                if not dry_run:
                    ...
                    location_met = journey.location_first_met or ""
                    formatted_events_en = [
                        e.format(location_met=location_met) for e in events_en
                    ]
                    formatted_events_de = [
                        e.format(location_met=location_met) for e in events_de
                    ]
                    formatted_events_fr = [
                        e.format(location_met=location_met) for e in events_fr
                    ]
                    ...
                    timeline_challenge.save(
                        update_fields=["options_en", "options_de", "options_fr"]
                    )
            ...
            for idx, challenge in enumerate(challenges):
                if challenge.challenge_type == "would_you_rather" and idx < len(wyr_de):
                    self._update_challenge_fields(
                        ...
                        question_fr=(
                            wyr_fr[idx].get("question", "") if idx < len(wyr_fr) else ""
                        ),
            ...
            for idx, challenge in enumerate(challenges):
                if idx < len(riddles_de):
                    self._update_challenge_fields(
                        ...
                        question_fr=(
                            riddles_fr[idx].get("question", "")
                            if idx < len(riddles_fr)
                            else ""
                        ),
                        success_de=riddles_de[idx].get("success", ""),
                        success_fr=(
                            riddles_fr[idx].get("success", "")
                            if idx < len(riddles_fr)
                            else ""
                        ),
                        dry_run=dry_run,
    ...
    def _update_from_challenges_array(
        self, challenges, content_de, content_fr, dry_run
    ):
    ...
    def update_chapter_rewards(
        self, chapter, content_de, content_fr, first_name, dry_run
    ):
