...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
            ...
            if dry_run:
                ...
                try:
                    ...
                    if success:
                        ...
                        error_count += 1
                        self.stdout.write(
                            self.style.ERROR(f"  Failed: users/{user_id}/")
                        )
                except Exception as e:
                    error_count += 1
                    self.stdout.write(
                        self.style.ERROR(f"  Error deleting users/{user_id}/: {str(e)}")
                    )
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"Would delete {deleted_count} orphaned folder(s)")
            )
