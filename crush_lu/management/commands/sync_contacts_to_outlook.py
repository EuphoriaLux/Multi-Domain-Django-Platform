...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--profile-id", type=int, help="Sync only a specific profile by ID"
        )
        parser.add_argument(
            "--delete-all",
            action="store_true",
    ...
    def _handle_single_profile(
        ...
        if not profile.phone_number:
            self.stdout.write(self.style.WARNING("  Skipped - no phone number\n"))
            return
    ...
    def _handle_full_sync(self, service: GraphContactsService, dry_run: bool):
        ...
        profiles_with_phone = (
            CrushProfile.objects.exclude(phone_number__isnull=True)
