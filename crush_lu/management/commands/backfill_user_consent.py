...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total_users == 0:
            self.stdout.write(
                self.style.SUCCESS("All users already have consent records")
            )
            return
            ...
            if dry_run:
                ...
                UserDataConsent.objects.create(
                    ...
                    crushlu_consent_date=(
                        user.date_joined if has_crushlu_profile else None
                    ),
                )
                created_count += 1
                ...
                if created_count % 100 == 0:
                    self.stdout.write(
                        f"Created {created_count}/{total_users} consent records..."
                    )

        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(f"Would create {total_users} consent records")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully created {created_count} consent records"
                )
            )
