...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--skip-azurite", action="store_true", help="Skip Azurite container setup"
        )
    ...
    def handle(self, *args, **options):
        ...
        if not profiles_only:
            ...
            results["activities"] = self.run_command_safe(
                "populate_global_activity_options"
            )
    ...
    def run_command_safe(self, command_name, **kwargs):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f"  Warning: {command_name} failed: {e}")
            )
            return False
