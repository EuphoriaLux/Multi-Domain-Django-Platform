...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--last-name", type=str, required=True, help="Last name of the special user"
        )
        parser.add_argument(
            "--date-met",
            type=str,
    ...
    def upload_media_to_reward(self, reward, file_path, media_type="photo"):
        ...
        if not os.path.exists(file_path):
            self.stdout.write(
                self.style.WARNING(f"  [WARN] File not found: {file_path}")
            )
            return False
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"  [OK] Uploaded {media_type}: {filename}")
            )
            return True
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"  [ERROR] Failed to upload {media_type}: {e}")
            )
            return False
    ...
    def handle(self, *args, **options):
        ...
        special_exp, created = SpecialUserExperience.objects.update_or_create(
            ...
            defaults={
                "is_active": True,
                # English
                "custom_welcome_title_en": self.get_text(
                    "en", "welcome_title", first_name=first_name
                ),
                "custom_welcome_message_en": self.get_text("en", "welcome_message"),
                # German
                "custom_welcome_title_de": self.get_text(
                    "de", "welcome_title", first_name=first_name
                ),
                "custom_welcome_message_de": self.get_text("de", "welcome_message"),
                # French
                "custom_welcome_title_fr": self.get_text(
                    "fr", "welcome_title", first_name=first_name
                ),
        ...
        if created:
            ...
            self.stdout.write(
                f"[OK] Updated existing Special User Experience for {first_name}"
            )
        ...
        self.create_all_chapters(
            journey, date_met, location_met, first_name, media_options
        )
    ...
    def create_all_chapters(
        self, journey, date_met, location_met, first_name, media_options=None
    ):
        ...
        self.create_chapter_3(
            journey, location_met, media_options.get("chapter3_image")
        )
        ...
        if challenges_en:
            ...
            JourneyChallenge.objects.update_or_create(
                ...
                defaults={
                    ...
                    # English
                    "question_en": riddle_en.get("question", "").format(
                        month=date_met.strftime("%B")
                    ),
                    "hint_1_en": riddle_en.get("hint_1", "").format(season=season_en),
                    "hint_2_en": riddle_en.get("hint_2", "").format(
                        month=date_met.strftime("%B")
                    ),
                    ...
                    # German
                    "question_de": riddle_de.get("question", "").format(
                        month=self.get_german_month(date_met.month)
                    ),
                    "hint_1_de": riddle_de.get("hint_1", "").format(season=season_de),
                    "hint_2_de": riddle_de.get("hint_2", "").format(
                        month=self.get_german_month(date_met.month)
                    ),
                    ...
                    # French
                    "question_fr": riddle_fr.get("question", "").format(
                        month=self.get_french_month(date_met.month)
                    ),
                    "hint_1_fr": riddle_fr.get("hint_1", "").format(season=season_fr),
                    "hint_2_fr": riddle_fr.get("hint_2", "").format(
                        month=self.get_french_month(date_met.month)
                    ),
    ...
    def create_chapter_5(self, journey, first_name, chapter5_image=None):
        ...
        reward, reward_created = JourneyReward.objects.update_or_create(
            ...
            defaults={
                ...
                "title_en": reward_en.get("title", ""),
                "message_en": reward_en.get("message", "").format(
                    first_name=first_name
                ),
                "title_de": reward_de.get("title", ""),
                "message_de": reward_de.get("message", "").format(
                    first_name=first_name
                ),
                "title_fr": reward_fr.get("title", ""),
                "message_fr": reward_fr.get("message", "").format(
                    first_name=first_name
                ),
            },
        )
    ...
    def create_chapter_6(self, journey, first_name):
        ...
        chapter, _ = JourneyChapter.objects.update_or_create(
            ...
            defaults={
                ...
                "theme_en": content_en.get("theme", ""),
                "story_introduction_en": content_en.get("story_introduction", ""),
                "completion_message_en": content_en.get(
                    "completion_message", ""
                ).format(first_name=first_name),
                "title_de": content_de.get("title", ""),
                "theme_de": content_de.get("theme", ""),
                "story_introduction_de": content_de.get("story_introduction", ""),
                "completion_message_de": content_de.get(
                    "completion_message", ""
                ).format(first_name=first_name),
                "title_fr": content_fr.get("title", ""),
                "theme_fr": content_fr.get("theme", ""),
                "story_introduction_fr": content_fr.get("story_introduction", ""),
                "completion_message_fr": content_fr.get(
                    "completion_message", ""
                ).format(first_name=first_name),
