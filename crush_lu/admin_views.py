...
def crush_admin_dashboard(request):
    ...
    legacy_stats = CrushProfile.objects.aggregate(
        ...
        step2_completed=Count(
            "id",
            filter=Q(completion_status__in=["step2", "step3", "step4", "submitted"]),
        ),
        step3_completed=Count(
            "id", filter=Q(completion_status__in=["step3", "step4", "submitted"])
        ),
    )
