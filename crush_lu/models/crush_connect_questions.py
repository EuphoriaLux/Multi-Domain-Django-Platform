"""Crush Connect's weekly Read-the-Photo question catalogue.

Each member picks three yes/no questions and privately records their own answer.
Connect Week participants guess those answers from a card; guesses are stored on
``ConnectCycleCard.answers_json`` and exposed only as anonymous aggregates.
...
class ConnectQuestion(models.Model):
    ...
    text = models.CharField(
        max_length=200,
        help_text=_(
            "Yes/no question, profile-owner POV (translated via modeltranslation)"
        ),
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text=_(
            "Inactive questions stop being offered but stay linked from history"
        ),
    )
...
class ConnectQuestionWeek(models.Model):
    ...
    The set is computed once per ISO week (deterministically, from the active
    catalogue) and pinned, so members
    ...
    def __str__(self):
        return (
            f"{self.iso_year}-W{self.iso_week:02d} ({self.questions.count()} questions)"
        )
...
class MemberGateQuestion(models.Model):
    ...
    owner_answer = models.BooleanField(
        help_text=_(
            "The member's own truthful yes/no — what guesses are scored against"
        ),
    )
