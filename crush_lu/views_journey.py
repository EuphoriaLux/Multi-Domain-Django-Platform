...
def journey_selector(request):
    ...
    journeys = JourneyConfiguration.objects.filter(
        special_experience=special_experience, is_active=True
    ...
    for journey in journeys:
        ...
        if journey.journey_type == "wonderland":
            ...
            icon = "🗺️"
            description = _(
                "A magical 6-chapter adventure through the Wonderland of You"
            )
        elif journey.journey_type == "advent_calendar":
...
def journey_map_wonderland(request):
    ...
    """
    logger.info(
        f"🎮 journey_map_wonderland called for user: {request.user.username} (first: '{request.user.first_name}', last: '{request.user.last_name}')"
    )
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Wonderland journey found: {journey.journey_name}, active: {journey.is_active}"
            )
        ...
        if created:
            logger.info(
                f"🎮 Started new journey for {request.user.username}: {journey.journey_name}"
            )
        ...
        for chapter in chapters:
            ...
            try:
                chapter_progress = ChapterProgress.objects.get(
                    journey_progress=journey_progress, chapter=chapter
                )
            ...
            if chapter.chapter_number == 1:
                ...
                try:
                    previous_chapter = chapters.get(
                        chapter_number=chapter.chapter_number - 1
                    )
                    previous_progress = ChapterProgress.objects.get(
                        journey_progress=journey_progress, chapter=previous_chapter
                    )
            ...
            chapter_data.append(
                {
                    ...
                    "is_current": chapter.chapter_number
        ...
        messages.error(
            request,
            _(
                "An error occurred loading your journey. Please contact support if this persists."
            ),
        )
...
def chapter_view(request, chapter_number):
    ...
    try:
        # Get user's journey
        journey_progress = (
            JourneyProgress.objects.filter(user=request.user)
        ...
        if chapter.requires_previous_completion and chapter.chapter_number > 1:
            previous_chapter = JourneyChapter.objects.get(
                journey=journey_progress.journey, chapter_number=chapter_number - 1
            )
            try:
                previous_progress = ChapterProgress.objects.get(
                    journey_progress=journey_progress, chapter=previous_chapter
                ...
                if not previous_progress.is_completed:
                    messages.warning(
                        request,
                        _("Please complete Chapter %(chapter)s first.")
        ...
        if (
            all_challenges_correct
            and challenges.exists()
            and not chapter_progress.is_completed
        ):
            ...
            journey_progress.current_chapter = max(
                journey_progress.current_chapter, chapter_number + 1
            )
            journey_progress.save()

            logger.info(
                f"✅ {request.user.username} completed Chapter {chapter_number}"
            )
        ...
        messages.error(
            request,
            _(
                "An error occurred loading Chapter %(chapter)s. Please try again or contact support."
            )
...
def _validate_timeline_structure(challenge, options, lang):
    ...
    """
    result = {"is_valid": False, "structure_type": None, "warnings": []}
        ...
        if isinstance(options["events"], list):
            ...
            for idx, event in enumerate(options["events"]):
                if not isinstance(event, dict):
                    ...
                    k.startswith("title_") or k.startswith("description_")
                    for k in event.keys()
                ):
...
def _get_timeline_events_with_fallback(challenge, requested_lang="en"):
    ...
    fallback_order = [requested_lang] + [
        lang for lang in supported_languages if lang != requested_lang
    ]
...
def challenge_view(request, chapter_number, challenge_id):
    ...
    try:
        ...
        chapter_progress = get_object_or_404(
            ChapterProgress, journey_progress=journey_progress, chapter=chapter
        )

        # Get the challenge
        challenge = get_object_or_404(
            JourneyChallenge, id=challenge_id, chapter=chapter
        )

        # Check if already completed
        existing_attempt = ChallengeAttempt.objects.filter(
            chapter_progress=chapter_progress, challenge=challenge, is_correct=True
        ).first()
        ...
        logger.error(
            f"❌ Error loading challenge {challenge_id} in chapter {chapter_number}: {e}",
            exc_info=True,
        )
        messages.error(
            request, _("An error occurred loading this challenge. Please try again.")
        )
...
def reward_view(request, reward_id):
    ...
    try:
        ...
        try:
            chapter_progress = ChapterProgress.objects.get(
                journey_progress=journey_progress, chapter=reward.chapter
            )

            if not chapter_progress.is_completed:
                messages.warning(
                    request, _("Complete all challenges to unlock this reward.")
                )
                return redirect(
                    "crush_lu:chapter_view",
                    chapter_number=reward.chapter.chapter_number,
                )
        except ChapterProgress.DoesNotExist:
            messages.warning(request, _("You must complete the chapter first."))
            return redirect(
                "crush_lu:chapter_view", chapter_number=reward.chapter.chapter_number
            )
        ...
        messages.error(
            request, _("An error occurred loading this reward. Please try again.")
        )
...
def certificate_view(request):
    ...
    try:
        ...
        journey_progress = (
            JourneyProgress.objects.filter(user=request.user, is_completed=True)
            .select_related("journey")
            .first()
        )
        ...
        if not journey_progress:
            messages.warning(
                request, _("Complete the journey to unlock your certificate.")
            )
            return redirect("crush_lu:journey_map")
        ...
        logger.error(
            f"❌ Error generating certificate for {request.user.username}: {e}",
            exc_info=True,
        )
        messages.error(
            request,
            _("An error occurred generating your certificate. Please contact support."),
        )
