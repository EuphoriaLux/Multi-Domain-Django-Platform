((this.workbox = this.workbox || {}),
    (this.workbox.backgroundSync = (function (t, e, s, n) {
            ...
            async getDb() {
                return (
                    this.t ||
                        ...
                        })("workbox-background-sync", 3, { upgrade: this.i })),
                    this.t
                );
            }
            i(t, e) {
        ...
        class E {
            constructor(t) {
                ((this.o = t), (this.u = new k()));
            }
            async pushEntry(t) {
                (delete t.id, (t.queueName = this.o), await this.u.addEntry(t));
            }
            async unshiftEntry(t) {
                const e = await this.u.getFirstEntryId();
                (e ? (t.id = e - 1) : delete t.id,
                    (t.queueName = this.o),
                    await this.u.addEntry(t));
            }
            async popEntry() {
                return this.h(await this.u.getLastEntryByQueueName(this.o));
            }
            async shiftEntry() {
                return this.h(await this.u.getFirstEntryByQueueName(this.o));
            }
            async getAll() {
                return await this.u.getAllEntriesByQueueName(this.o);
            }
            async size() {
                return await this.u.getEntryCountByQueueName(this.o);
            }
            async deleteEntry(t) {
                await this.u.deleteEntry(t);
            }
            async h(t) {
        ...
        class x {
            ...
            toObject() {
                ...
                return (
                    (t.headers = Object.assign({}, this.l.headers)),
        ...
        class A {
            ...
            async getAll() {
                const t = await this.q.getAll(),
                    e = Date.now(),
                    s = [];
                for (const n of t) {
                    const t = 60 * this.B * 1e3;
                    e - n.timestamp > t ? await this.q.deleteEntry(n.id) : s.push(S(n));
                }
                return s;
            }
            async size() {
                return await this.q.size();
            }
            async R({ request: t, metadata: e, timestamp: s = Date.now() }, n) {
                const r = {
                ...
                switch ((e && (r.metadata = e), n)) {
                    case "push":
                        await this.q.pushEntry(r);
                        break;
                    case "unshift":
                        await this.q.unshiftEntry(r);
                }
                this.m ? (this.p = !0) : await this.registerSync();
            }
            async v(t) {
                const e = Date.now();
                let s;
                switch (t) {
                    case "pop":
                        s = await this.q.popEntry();
                        break;
                    case "shift":
                        s = await this.q.shiftEntry();
                }
                if (s) {
                    const n = 60 * this.B * 1e3;
                    return e - s.timestamp > n ? this.v(t) : S(s);
                }
            }
                ...
                for (; (t = await this.shiftRequest()); )
                    try {
                        ...
                        throw (
                            ...
                            new e.WorkboxError("queue-replay-failed", { name: this.g })
                        );
                    }
            }
            async registerSync() {
                if ("sync" in self.registration && !this.I)
                    try {
                        await self.registration.sync.register(`${v}:${this.g}`);
                    } catch (t) {}
            }
            k() {
                "sync" in self.registration && !this.I
