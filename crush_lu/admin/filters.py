...
class CoachAssignmentFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "has_coach":
            return queryset.filter(profilesubmission__coach__isnull=False).distinct()
        elif self.value() == "no_coach":
            return queryset.filter(profilesubmission__coach__isnull=True).distinct()
        elif self.value() == "not_submitted":
            return queryset.filter(profilesubmission__isnull=True)
        return queryset
...
class SubmissionWorkflowFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "urgent":
            ...
            return queryset.filter(status="pending", submitted_at__lt=cutoff_24h)
        elif self.value() == "new":
            # Pending and submitted < 24h ago
            return queryset.filter(status="pending", submitted_at__gte=cutoff_24h)
...
class PhoneVerificationFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "verified":
            ...
            return queryset.filter(
                phone_verified=False, phone_number__isnull=False
            ).exclude(phone_number="")
        elif self.value() == "no_phone":
            from django.db.models import Q

            return queryset.filter(Q(phone_number__isnull=True) | Q(phone_number=""))
        return queryset
...
class AgeRangeFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        return queryset.filter(
            date_of_birth__gt=min_birth, date_of_birth__lte=max_birth
        )
        ...
        if self.value() == "full":
            return queryset.annotate(
                confirmed_count=Count(
                    "eventregistration",
                    filter=models.Q(eventregistration__status="confirmed"),
                )
...
class HasMessagesFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "yes":
            return queryset.annotate(message_count=Count("messages")).filter(
                message_count__gt=0
            )
        elif self.value() == "no":
            return queryset.annotate(message_count=Count("messages")).filter(
...
class DaysSinceSignupFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "new":
            ...
            return queryset.filter(
                created_at__gte=cutoff_start, created_at__lt=cutoff_end
            )
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "fresh":
            ...
            return pending_qs.filter(
                submitted_at__gte=cutoff_start, submitted_at__lt=cutoff_end
            )
...
class ProfileCompletenessFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "complete":
            ...
            return (
                queryset.filter(photo_1__isnull=False)
                .exclude(photo_1="")
                .exclude(Q(bio__isnull=True) | Q(bio=""))
                .exclude(Q(interests__isnull=True) | Q(interests=""))
            )
        elif self.value() == "missing_photos":
            # No photos at all
            return queryset.filter(Q(photo_1__isnull=True) | Q(photo_1=""))
        elif self.value() == "missing_bio":
            return queryset.filter(Q(bio__isnull=True) | Q(bio=""))
        elif self.value() == "missing_interests":
            return queryset.filter(Q(interests__isnull=True) | Q(interests=""))
...
class EmailVerificationStatusFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "verified":
            ...
            return queryset.filter(
                ~Exists(EmailAddress.objects.filter(user_id=OuterRef("user_id")))
            )
        return queryset
...
class PrivacySettingsFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "high_privacy":
            ...
            return queryset.filter(
                show_full_name=True, show_exact_age=True, blur_photos=False
            )
        return queryset
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "never_submitted":
            ...
            return queryset.filter(
                Exists(
                    ProfileSubmission.objects.filter(
                        profile_id=OuterRef("id"), status="rejected"
                    )
                )
            )
            ...
            return queryset.filter(
                Exists(
                    ProfileSubmission.objects.filter(
                        profile_id=OuterRef("id"), status="revision_requested"
                    )
                )
            )
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "no_connections":
            ...
            return queryset.filter(
                Exists(
                    EventConnection.objects.filter(
                        requester__crushprofile__id=OuterRef("id"), status="pending"
                    )
                )
            )
            ...
            return queryset.filter(
                Exists(
                    EventConnection.objects.filter(
                        recipient__crushprofile__id=OuterRef("id"), status="pending"
                    )
                )
            )
