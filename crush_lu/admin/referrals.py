...
class ReferralCodeAdmin(admin.ModelAdmin):
    ...
    def get_referrer_link(self, obj):
        profile_url = reverse(
            "crush_admin:crush_lu_crushprofile_change", args=[obj.referrer.pk]
        )
        return format_html('<a href="{}">{}</a>', profile_url, obj.referrer.user.email)

    get_referrer_link.short_description = "Referrer"

    def get_referral_counts(self, obj):
        converted = obj.attributions.filter(
            status=ReferralAttribution.Status.CONVERTED
        ).count()
        pending = obj.attributions.filter(
