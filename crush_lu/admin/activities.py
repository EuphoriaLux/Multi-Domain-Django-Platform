...
class GlobalActivityOptionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Statistics", {"fields": ("get_total_votes",)}),
...
class EventActivityVoteAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Vote Details", {"fields": ("event", "user", "selected_option")}),
        ("Metadata", {"fields": ("voted_at",)}),
    )
...
class EventVotingSessionAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "voting_end_time",
        "total_votes",
        "winning_presentation_style",
        "winning_speed_dating_twist",
    )
    ...
    def start_voting_session(self, request, queryset):
        ...
        django_messages.success(
            request, _("Started voting for %(count)d session(s)") % {"count": updated}
        )
    ...
    def end_voting_session(self, request, queryset):
        ...
        django_messages.success(
            request,
            _("Ended voting and calculated winners for %(count)d session(s)")
            % {"count": updated},
        )
...
class PresentationRatingAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Rating Details", {"fields": ("event", "presenter", "rater", "rating")}),
        ("Metadata", {"fields": ("rated_at",)}),
    )
...
class SpeedDatingPairAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Pairing Details", {"fields": ("event", "user1", "user2", "round_number")}),
        (
            "Matching Data",
