...
class SpecialUserExperienceAdmin(admin.ModelAdmin):
    ...
    actions = [
        "activate_experiences",
        "deactivate_experiences",
        "generate_wonderland_journey",
    ...
    def generate_journey_view(self, request, object_id):
        ...
        existing_wonderland = JourneyConfiguration.objects.filter(
            special_experience=special_exp, journey_type="wonderland"
        ).exists()
        existing_advent = JourneyConfiguration.objects.filter(
            special_experience=special_exp, journey_type="advent_calendar"
        ).exists()
    ...
    def _create_wonderland_journey(self, request, special_exp):
        ...
        if existing:
            ...
            return HttpResponseRedirect(
                reverse("crush_admin:crush_lu_specialuserexperience_changelist")
            )
        ...
        try:
            ...
            command.create_all_chapters(
                journey, parsed_date, location_met, special_exp.first_name
            )
            ...
            django_messages.error(
                request,
                _("Error generating Wonderland journey: %(error)s") % {"error": str(e)},
            )
    ...
    def _create_advent_calendar(self, request, special_exp):
        ...
        title = (
            request.POST.get("advent_title", "Your Advent Calendar")
            or f"{special_exp.first_name}'s Advent Calendar"
        )
        ...
        try:
            ...
            DEFAULT_DOORS = [
                ...
                },
                {
                    ...
                    "type": "gift_teaser",
                    "qr": "required",
                    "teaser": "Something special awaits...",
                    "icon": "🎁",
                },
                {
                    "day": 7,
                ...
                },
                {
                    ...
                    "type": "gift_teaser",
                    "qr": "required",
                    "teaser": "The countdown continues",
                    "icon": "🎁",
                },
                {
                    "day": 21,
            ...
            calendar = AdventCalendar.objects.create(
                ...
                calendar_description=welcome
                or f"Welcome to your personal Advent Calendar, {special_exp.first_name}! Each day unlocks a new surprise just for you.",
            )
            ...
            if generate_qr:
                ...
                if user:
                    for door in calendar.doors.filter(
                        qr_mode__in=["required", "bonus"]
                    ):
            ...
            django_messages.error(
                request,
                _("Error generating Advent Calendar: %(error)s") % {"error": str(e)},
            )
            print(f"Error details: {error_detail}")
    ...
    def get_source_display(self, obj):
        ...
        return mark_safe(
            '<span style="background: #666; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px;">📋 Manual</span>'
        )
    ...
    def get_journeys_status(self, obj):
        ...
        for journey in journeys:
            if journey.journey_type == "wonderland":
                badges.append(
                    '<span style="background: #9B59B6; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px; margin-right: 4px;">🎭 Wonderland</span>'
                )
            elif journey.journey_type == "advent_calendar":
                badges.append(
                    '<span style="background: #C41E3A; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px; margin-right: 4px;">🎄 Advent</span>'
                )
            else:
                badges.append(
                    f'<span style="background: #666; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px; margin-right: 4px;">📖 {journey.journey_type}</span>'
                )
    ...
    def get_journey_status(self, obj):
        ...
        try:
            ...
            challenge_count = sum(
                chapter.challenges.count() for chapter in journey.chapters.all()
            )
            journey_url = reverse(
                "crush_admin:crush_lu_journeyconfiguration_change", args=[journey.id]
            )
            return format_html(
    ...
    def activate_experiences(self, request, queryset):
        ...
        django_messages.success(
            request, _("Activated %(count)s special experience(s)") % {"count": updated}
        )
    ...
    def deactivate_experiences(self, request, queryset):
        ...
        django_messages.success(
            request,
            _("Deactivated %(count)s special experience(s)") % {"count": updated},
        )
    ...
    def generate_wonderland_journey(self, request, queryset):
        ...
        if queryset.count() != 1:
            django_messages.error(
                request,
                "Please select exactly ONE special experience to generate a journey for.",
            )
            return
        ...
        return HttpResponseRedirect(
            reverse(
                "crush_admin:crush_lu_specialuserexperience_generate_journey",
                args=[special_exp.id],
            )
        )
    ...
    def generate_advent_calendar(self, request, queryset):
        ...
        if queryset.count() != 1:
            django_messages.error(
                request,
                "Please select exactly ONE special experience to generate an Advent Calendar for.",
            )
            return
