...
class EventConnectionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Recipient Info", {"fields": ("recipient_consents_to_share",)}),
        (
            "Coach Facilitation",
...
class ConnectionMessageAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        ...
        "sender__email",
        "message",
        "connection__event__title",
    )
    autocomplete_fields = ["sender"]
    readonly_fields = ("sent_at", "read_at")
    fieldsets = (
        ("Message Details", {"fields": ("connection", "sender", "message")}),
        ("Moderation", {"fields": ("is_coach_message", "coach_approved")}),
        ("Timestamps", {"fields": ("sent_at", "read_at")}),
    )
    ...
    def get_queryset(self, request):
        ...
        qs = super().get_queryset(request)
        return qs.select_related(
            "connection__requester",
            "connection__recipient",
            "connection__event",
            "sender",
        )
