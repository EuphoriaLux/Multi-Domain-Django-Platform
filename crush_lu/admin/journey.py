...
class JourneyConfigurationAdmin(TranslationAdmin):
    ...
    search_fields = (
        "journey_name",
        "special_experience__first_name",
        "special_experience__last_name",
        "final_message",
    )
    ...
    def activate_journeys(self, request, queryset):
        ...
        django_messages.success(
            request, _("Activated {count} journey(s)").format(count=updated)
        )
    ...
    def deactivate_journeys(self, request, queryset):
        ...
        django_messages.success(
            request, _("Deactivated {count} journey(s)").format(count=updated)
        )
    ...
    def duplicate_journey(self, request, queryset):
        ...
        if queryset.count() != 1:
            django_messages.error(
                request, _("Please select exactly one journey to duplicate")
            )
            return
        ...
        django_messages.info(
            request,
            _(
                "Duplication feature coming soon! Would create a copy of '{name}'"
            ).format(name=original.journey_name),
        )
...
class JourneyChapterAdmin(TranslationAdmin):
    ...
    fieldsets = (
        ...
        ("📖 Story & Theme", {"fields": ("story_introduction", "background_theme")}),
        (
            "⚙️ Settings",
            {
                "fields": (
...
class JourneyChallengeAdmin(TranslationAdmin):
    ...
    fieldsets = (
        ...
        ("🏆 Scoring & Feedback", {"fields": ("points_awarded", "success_message")}),
    )
...
class JourneyRewardAdmin(TranslationAdmin):
    ...
    fieldsets = (
        ("📍 Reward Location", {"fields": ("chapter", "reward_type", "title")}),
...
class JourneyProgressAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("👤 User & Journey", {"fields": ("user", "journey")}),
        (
            "📊 Progress Tracking",
            {
                "fields": (
        ...
        (
            "🗓️ Timestamps",
            {"fields": ("started_at", "last_activity"), "classes": ("collapse",)},
        ),
    )
...
class ChapterProgressAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("📖 Chapter Info", {"fields": ("journey_progress", "chapter")}),
        (
            "📊 Progress",
            {
                "fields": (
        ...
        ("🗓️ Timestamps", {"fields": ("started_at", "completed_at")}),
    )
...
class ChallengeAttemptAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("📝 User Response", {"fields": ("user_answer",)}),
        ("💡 Hints Used", {"fields": ("hints_used",)}),
        ("🗓️ Timestamp", {"fields": ("attempted_at",)}),
    )
    ...
    def export_chapter2_responses(self, request, queryset):
        ...
        chapter2_attempts = (
            queryset.filter(challenge__chapter__chapter_number=2)
            .select_related("chapter_progress__journey_progress__user", "challenge")
            .order_by(
                "chapter_progress__journey_progress__user", "challenge__challenge_order"
            )
        )
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="chapter2_responses_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        )
        ...
        self.message_user(
            request,
            _("Exported {count} Chapter 2 responses.").format(
                count=chapter2_attempts.count()
            ),
    ...
    export_chapter2_responses.short_description = _(
        "📊 Export Chapter 2 Questionnaire Responses (CSV)"
    )
...
class RewardProgressAdmin(admin.ModelAdmin):
    ...
    search_fields = ("journey_progress__user__username", "reward__title")
    readonly_fields = ("started_at", "completed_at")
...
class JourneyGiftAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("🎁 Gift Information", {"fields": ("gift_code", "status", "qr_code_image")}),
        ("👤 Sender", {"fields": ("sender", "sender_message")}),
        (
    ...
    def mark_as_expired(self, request, queryset):
        ...
        django_messages.success(
            request, _("Marked {count} gift(s) as expired").format(count=updated)
        )
