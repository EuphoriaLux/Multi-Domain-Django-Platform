...
class CrushCoachAdmin(TranslationAdmin):
    ...
    fieldsets = (
        ...
        ("Settings", {"fields": ("is_active", "max_active_reviews")}),
...
class CrushProfileAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        awaiting_review = CrushProfile.objects.filter(
            is_approved=False, completion_status__in=["step4", "submitted"]
        ).count()
        ...
        high_privacy = CrushProfile.objects.filter(
            show_full_name=False, show_exact_age=False, blur_photos=True
        ).count()
    ...
    def get_assigned_coach(self, obj):
        ...
        if submissions is not None:
            ...
            submission = (
                ProfileSubmission.objects.filter(profile=obj)
        ...
        if submission.coach:
            coach_url = reverse(
                "crush_admin:crush_lu_crushcoach_change", args=[submission.coach.pk]
            )
            status_colors = {
                "pending": "#ffc107",
                "approved": "#28a745",
    ...
    def get_quick_status_summary(self, obj):
        ...
        comp_icon, comp_text = completion_map.get(
            obj.completion_status, ("❓", obj.completion_status or "Unknown")
        )
