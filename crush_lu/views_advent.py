...
def advent_calendar_view(request):
    ...
    try:
        ...
        if not special_experience:
            messages.warning(
                request, _("No special experience found for your account.")
            )
        ...
        if not journey or not journey.is_active:
            messages.info(
                request, _("No Advent Calendar is currently available for you.")
            )
            return redirect("crush_lu:home")
    ...
    try:
        ...
        if not calendar.is_door_available(door_number):
            ...
            if current_day is None:
                messages.info(
                    request, _("The Advent Calendar is only available in December.")
                )
            elif door_number > current_day:
                messages.info(
                    request,
                    _("Door %(door)s will be available on December %(day)s.")
                    % {"door": door_number, "day": door_number},
                )
            return redirect("crush_lu:advent_calendar")
        ...
        if door.requires_qr_to_open() and not progress.has_scanned_qr(door_number):
            messages.info(
                request, _("Scan the QR code on your physical gift to open this door!")
            )
            return redirect("crush_lu:advent_calendar")
        ...
        template_name = template_map.get(
            door.content_type, "crush_lu/advent/door_default.html"
        )
...
def scan_qr_code(request, token):
    ...
    try:
        ...
        if not qr_token.is_valid():
            ...
            return redirect(
                "crush_lu:advent_door", door_number=qr_token.door.door_number
            )
        ...
        if qr_token.redeem():
            ...
            logger.info(
                f"{request.user.username} scanned QR for door {door.door_number}"
            )

            if door.requires_qr_to_open():
                messages.success(
                    request,
                    _("Door %(door)s unlocked! Open it to see your gift.")
...
def get_advent_status(request):
    ...
    try:
        ...
        if not special_experience:
            return JsonResponse(
                {"success": False, "error": "No experience found"}, status=404
            )

        journey = special_experience.advent_calendar_journey
        if not journey:
            return JsonResponse(
                {"success": False, "error": "No calendar found"}, status=404
            )
        ...
        return JsonResponse(
            {
                "success": False,
                "error": "An error occurred while loading calendar status.",
            },
            status=500,
        )
...
def open_door_api(request):
    ...
    try:
        ...
        if not door_number or door_number < 1 or door_number > 24:
            return JsonResponse(
                {"success": False, "error": "Invalid door number"}, status=400
            )
        ...
        if door.requires_qr_to_open() and not progress.has_scanned_qr(door_number):
            return JsonResponse(
                {"success": False, "error": "QR scan required", "requires_qr": True},
                status=403,
            )
        ...
        return JsonResponse(
            {"success": False, "error": "An error occurred while opening this door."},
            status=500,
        )
