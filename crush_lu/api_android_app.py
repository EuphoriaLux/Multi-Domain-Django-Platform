...
from .models import AndroidNativeAuthCode
...
def android_auth_handoff(request):
    ...
    code = AndroidNativeAuthCode.issue(request.user, redirect_uri, request=request)
...
def android_auth_complete(request, code):
    ...
    user = AndroidNativeAuthCode.consume(code)
    if not user:
        return JsonResponse(
