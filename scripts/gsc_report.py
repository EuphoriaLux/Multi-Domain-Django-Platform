...
def main():
    ...
    monthly_agg = defaultdict(lambda: {"clicks": 0, "impressions": 0, "ctr_sum": 0.0, "pos_sum": 0.0, "count": 0})
    ...
    # 5. Country Breakdown
    country_rows = query_gsc(service, start_str, end_str, ["country"], row_limit=row_limit)
    countries = []
    ...
    if args.csv:
        ...
        if data.get("summary"):
            write_csv("gsc_summary.csv", list(data["summary"].keys()), [data["summary"]])

        if monthly_trend:
            write_csv("gsc_monthly_trend.csv",
                       ["month", "clicks", "impressions", "avg_ctr", "avg_position"],
                       monthly_trend)

        if queries:
            write_csv("gsc_top_queries.csv",
                       ["query", "clicks", "impressions", "ctr", "position"],
                       queries)

        if pages:
            write_csv("gsc_top_pages.csv",
                       ["page", "clicks", "impressions", "ctr", "position"],
                       pages)

        if countries:
            write_csv("gsc_countries.csv",
    ...
    if monthly_trend:
        table_rows = [[m["month"], m["clicks"], m["impressions"], m["avg_ctr"], m["avg_position"]] for m in monthly_trend]
        print_table(
            "MONTHLY TREND",
    ...
    if queries:
        table_rows = [[q["query"], q["clicks"], q["impressions"], q["ctr"], q["position"]] for q in queries[:25]]
        print_table(
            "TOP SEARCH QUERIES",
            ["Query", "Clicks", "Impressions", "CTR", "Position"],
            table_rows,
    ...
    if pages:
        table_rows = [[p["page"], p["clicks"], p["impressions"], p["ctr"], p["position"]] for p in pages[:15]]
        print_table(
            "TOP PAGES",
            ["Page", "Clicks", "Impressions", "CTR", "Position"],
            table_rows,
    ...
    if countries:
        table_rows = [[c["country"], c["clicks"], c["impressions"], c["ctr"], c["position"]] for c in countries[:15]]
        print_table(
            "COUNTRY BREAKDOWN",
            ["Country", "Clicks", "Impressions", "CTR", "Position"],
            table_rows,
