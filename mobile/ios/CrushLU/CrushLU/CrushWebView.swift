...
struct CrushWebView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler, PKAddPassesViewControllerDelegate {
        ...
        func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
            ...
            // The in-flight download has reported back either way; let the next
            // tap through.
        ...
        func presentAddPassController(for url: URL) {
            ...
            // backend now claims them atomically, so the loser adopts the
            // winner's values, but there is still nothing to gain from two
            // downloads and two native sheets. Cleared when the bridge reports
            // back (success or failure).
            guard !isDownloadingPass else { return }
            isDownloadingPass = true
            pendingPassURL = url
        ...
        /// True between kicking off a `.pkpass` fetch and the bridge reporting
        /// back, so a double-tap cannot start a second download.
        private var isDownloadingPass = false
