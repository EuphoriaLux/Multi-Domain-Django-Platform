...
def dashboard(request):
    ...
    # If no overlap with current dates, use last N days of available data
    ...
    # Get summary statistics using actual usage dates
    ...
    # Month-to-date calculation - use actual data range
    ...
    # Year-to-date calculation - use actual data range
...
def subscription_view(request):
    ...
    # Get all subscriptions with costs using actual usage dates
    ...
    context = {
        ...
        'subscriptions_list': subscriptions_list,  # For table iteration
...
def service_breakdown(request):
    ...
    # Build base queryset with usage date filtering
    base_queryset = CostRecord.objects.annotate(
        usage_date=TruncDate('charge_period_start')
    ).filter(
        usage_date__gte=start_date,
    ...
    total_cost = base_queryset.aggregate(
        total=Sum('billed_cost')
    )['total'] or 1  # Avoid division by zero

    # Get available subscriptions for filter (filtered by date range)
    ...
    # Convert only top 10 for chart JSON
    ...
    context = {
        ...
        'services': services_chart_data,  # For chart (top 10 only)
        'services_table': services_list,  # For table (all services)
        'total_cost': total_cost,  # For percentage calculations
    }
...
def resource_explorer(request):
    ...
    # Get resource costs - group by resource_name only to avoid duplicates
    # Some resources (like DNS zones) may appear in multiple resource groups
    resources = base_queryset.values(
        'resource_name'
    ).annotate(
        total_cost=Sum('billed_cost'),
        record_count=Count('id'),
        # Get the first value for display purposes
    ...
    # Get available filters (filtered by date range)
...
def trigger_import(request):
    ...
    if request.method == 'POST':
        # This would trigger import in background
    ...
    # Get incomplete exports (completed with 0 records)
    incomplete_exports = CostExport.get_incomplete_exports()

    # Get recent import history (exclude incomplete exports from main list)
...
def faq(request):
    ...
    """
    # Get latest export info for display
    latest_export = CostExport.objects.filter(
        import_status='completed'
